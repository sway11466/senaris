...
**カメラが行動中のユニットを追う**（優先度：中）

- 背景：敵AIの手番で「何が起きたか」を追えることは戦術の手応えに直結する（[uiux.md](gdd/uiux.md) 移動の見せ方）。移動アニメは入ったが、行動が画面外で起きれば結局見えない＝可読性の残り半分。現状カメラはプレイヤーの操作でしか動かず、AIの手を自動で追わない。
- 対応：AI手番の各手の前に、行動ユニット（と対象）が画面に入るようカメラを寄せる。既存のカメラリグ（俯角固定・`_fit_board`／ズーム）を流用し、盤外へ出ない範囲でパンする。移動アニメ・戦闘演出の待ち（`move_pace`／`combat_pace`）と同じテンポ制御に乗せる。プレイヤー操作中は介入しない。
- 該当：`presentation/board/hex_board_3d.gd`（カメラリグ）・`application/match_controller.gd`（手番のテンポ）・`doc/gdd/uiux.md`。

### feature-16

**移動アニメの速度設定と適用範囲の拡張**（優先度：低）
