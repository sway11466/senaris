...
**戦闘立ち絵のキャンバスにレッドドラゴンが収まらない**（優先度：低）

- 背景：`tools/gen_unit_combat.ps1` は 512四方・BaseHeight 384＝倍率 1.333 までしか収まらないが、dragon の `combat_scale` は 1.4（538px）で上端26pxが欠ける。`-extent` は黙って切り落とすため、切れを検出する警告は入れた（生成時に出る）が、キャンバス自体の決着はまだ。map 側（384四方）は対処済み。
- 対応：Canvas を 640 に上げる（倍率1.66まで）のが第一候補。戦闘シーンは正方キャンバスを枠へ `KEEP_ASPECT` で嵌めるので、キャンバスだけ広げると全員が一律小さく見える＝枠取りと一緒に決める必要がある。戦闘シーンの寸法確定（feature-22）と同時にやる。変えるときは既存の立ち絵を `all` で作り直す。
- 併発（map 側の残り）：`gen_unit_map.ps1` は `-resize "x<高さ>"` で高さしか指定しておらず横幅に上限が無い。翼を広げた竜のような横長の絵は倍率と無関係に左右が切れる（警告は出る）。内接指定に変えれば切れないが横長ユニットの背が低くなる＝大小関係が変わるので、竜の master ができて必要幅が分かってから決める。
- 該当：`tools/gen_unit_combat.ps1`（`$Canvas`）・`tools/gen_unit_map.ps1`（`-resize`）・`presentation/combat/combat_scene.gd`（枠）・`doc/art/units.md` 3.3。着手の引き金＝レッドドラゴンの絵を作るとき、または feature-22 に入るとき。
...
- 対応（残・4c/Phase 5）：複数スロットUI（枠一覧・上書き確認・スロット別ファイル）と、ターン毎オートセーブ（中断セーブの応用＝手番開始/終了で自動保存・別枠）。
