...
**uiux.md の全体レビュー**
- ゴール：uiux.md が「プレイヤーにどう見えるか」の規則だけを述べていて、実装の都合（情報板と会話パネルが別物、など）を根拠にした条文が残っていない。
- 背景：情報板を動かしても会話パネルは既定の場所に固定、と書いてあった（2026-09-07 に発見）。プレイヤーには同じ板に見えるものが別々に振る舞う規則を、実装が別物である理由で正当化している。同種の条文が他にも紛れている可能性が高い。
- 対応：uiux.md を頭から読み、各条文を「プレイヤーの体験として妥当か」「実装の都合を規則にしていないか」で点検する。要らない条文は消し、体験として決め直す条文はオーナーと1件ずつ判断する。情報板と会話パネルの位置・動きの共有は決定済み（uiux.md 移動）。
- 該当：`doc/gdd/uiux.md`。
...
- 対応：各冒険譚の `campaign.json` の `file` 欄とファイル名を `goblin-raid-st2.json` のように改名し、doc・テスト・ツールのパス参照を追従させる。
- 考慮外：マニフェストの `id`（`st2`）・翻訳キー（`t1.st2.*`）・進捗セーブのキーは変えない。
- 該当：`godot/data/stages/tutorial1-goblin-raid/`・`tutorial2-undead-rush/`・`tutorial3-dragon-hunt/`・`bounty1-goblin-horde/`、各 `campaign.json`、パスを持つ doc・`godot/tests/`。
