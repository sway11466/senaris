...
次回採番: bug=5 / feature=99 / refactoring=1.
...
- 背景：右の情報パネルは固定寸法（`UiLayout.RIGHT_BOX`＝464×532）で、盤エリアの右側を常時占めている。盤を広く見たい場面で大きさが気になる。常時小さくするほか、畳む・縮める切り替え式（コンパクトモード）も候補。パネルの矩形から位置を算出している物（タイトルロゴ・残りターン板）は矩形を動かせば追従する作りになっている（[uiux.md](gdd/uiux.md) タイトルロゴ）。
...
**ズームイン／アウトをメニューに置く（マウス操作を括弧書きで併記）**
- ゴール：メニューからズームイン／ズームアウトができ、項目のラベルにマウスでの操作が括弧書きで添えてある。
- 背景：ズームは現状 Ctrl＋ホイール（トラックパッドはピンチ）のみで、素のマウスホイールはパンに割り当てている（[uiux.md](gdd/uiux.md) 盤入力の詳細・既知の制約）。操作を知らないプレイヤーはズームに到達できない。メニューに項目を置けば機能への到達と操作の周知（「ズームイン（Ctrl＋ホイール）」のような括弧書き）を兼ねられる。
- 対応：置き場所（システムメニュー内か、常時HUDのボタンか）は未検討。同じく uiux.md 既知の制約に挙がっているホイール割り当ての切替（マウス運用時はホイール＝ズーム）と合わせて検討する。
- 該当：`godot/presentation/ui/hud.gd`・`godot/presentation/board/hex_board_3d.gd`・`doc/gdd/uiux.md`。
...
- ユニットのドラッグ移動：駒をドラッグして移動先へ運べる操作の案。現仕様は「ユニット上からのドラッグはパンしない＝ドラッグでユニットを動かす機能は持たない」と明記して外している（[uiux.md](gdd/uiux.md) カメラ）。採用するなら仕様変更で、クリック／ドラッグのしきい値判別・タッチの1本指パンとの衝突を含めて操作モデルを再設計する。
