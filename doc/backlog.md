...
次回採番: bug=2 / feature=43 / refactoring=9
...
**ステージ難易度の数値化（ユニット戦闘力の機械算出）**（優先度：中）

- 背景：[map_patterns.md](gdd/map_patterns.md) の難易度は帯（易/並/難/激）＋帯内位置（下/中/上）の定性評価で、ステージ間の比較や難易度カーブの検証に使える数値がない。数値化の第一歩として、`unit_type.csv` のステータスからユニットごとの戦闘力を機械的に算出する式を設計した。
- 算出式（現時点）：

```
# 攻撃
effective_atk = max(atk_ground, atk_air) / (1 - pierce)

# 防御（射程補正＋飛行補正）
range_mult    = 1 + (max_range - 1) × 0.2
flight_mult   = 飛行なら 1.5 / それ以外 1.0
effective_def = defense × range_mult × flight_mult

# 移動補正（攻防両方に掛かる）
move ≤ 4 : move_mult = 1 - (5 - move) × 0.05
move 5-6 : move_mult = 1.0
move ≥ 7 : move_mult = 1 + (move - 6) × 0.05

combat_power = (effective_atk × move_mult) × (effective_def × move_mult)
```

- 根拠：戦闘式 `A²/(A²+D²)` から、1v1で勝つ条件は `atk×def` の積が大きい側。pierce は相手防御を `×(1-pierce)` にするので実効攻撃に折り込む。射程が長いほど反撃を受けず被弾機会が減る＝実効防御の底上げ。飛行は `atk_air=0` の敵（歩兵・占領兵・重装）から殴られない＝防御の底上げ。移動 5-6 を中立帯とし、鈍足はペナルティ、高速はボーナス。
- 代表値（上位）：ドラゴン 8203／ペガサス 6844／ウォーロード 4900／ジャガーノート 4624／ヴァルキリー 4536／マジックナイト 3600／サムライ 3500／ウィザード 3466。
- 残課題：
  - **対地/対空の扱い**：`max()` だとペガサス（`atk_ground=0`）が地上を殴れないのに高く出る。重み付け平均か、対地0のペナルティが要る。
  - **特殊役割**：輸送（capacity）・占領（can_capture）・攻撃後移動（move_after_attack）の価値が未反映。戦闘力とは別の「有用度」軸を足すか、戦闘力に折り込むか。
