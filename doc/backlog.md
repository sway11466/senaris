...
- 仕様から変えたこと：吊り看板は作らなかった。絵に文字を入れないのが全アセット共通のルールなので、タイトルは UI 側で載せる。またタイトル専用の曲は作らず、作品の顔となる旋律は `menu` が担う（店内のメニュー表示から鳴らす。[bgm.md](audio/bgm.md)）。
- 対応（残り）：メニューは店内で出す。起動で扉が開く動画をそのまま再生（スキップ可）し、入り終わりの店内静止画（動画の最終コマを焼いたもの＝[menu.md](art/menu.md) §5）で止め、左の卓を人物ごと不透明の板で覆ってメニュー項目を載せる（下が透けると読みにくいので板は不透明）。板はセレクトの `board`、ボタンは `plank` を流用＝セレクトと同族の手触りになる。スキップ入力ではメニューを即出す＝Press any key の一拍を挟まない方針は保たれる（入力が空振りしない）。BGM は動画中〜店内表示までざわめき（`title`）のままとし、メニュー表示と同時に `menu` へクロスフェード（[bgm.md](audio/bgm.md)）。「はじめる」は扉の動画ではなく、セレクトへの簡素な繋ぎ（フェード等）にする。
- 決めていないこと：タイトルロゴを画面のどこに置くか（店内の絵は左をメニュー板・右をカウンターの主人が塞ぐので、上端の梁の暗い帯が候補）。
