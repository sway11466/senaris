...
- 対応：`godot/tools/` に画面ツールとして置く（索引 → [tools.md](tech/tools.md)）。開くのは実機の `user://` のファイルだけ（エディタからデバッグ起動したときに読み書きするもの）＝ファイル選択は持たない。書き込みはストア（`ProgressStore` ほか）を通す＝版と形式の責任をストアに残し、ツールは値だけを持つ。ストアの書き口は記録用（ランクは上位のときだけ・所要時間は短いときだけ）なので、任意の値に置く・消す口を足す。
  ...
  - 名簿：クリア済みにしたステージの控え（[campaigns.md](gdd/campaigns.md) 名簿）を書く。候補は、そのステージから `roster_from` をさかのぼって通った各ステージのJSONにある `actor` 駒＝`player` の駒と拠点の控え（`garrison`・中立の勧誘対象）の両方。1体ずつ「在籍」のチェック・Lv・兵数を指定する。チェックを外す＝加入しなかった、チェック入りで兵数0＝加入して離脱、で名簿の3つの状態を表す（加入の有無は名簿に載っているかどうかそのもので、別のフラグは持たない）。既定は全員チェック入り・Lv1・満員。
  - クロニクル：個別に見るユースケースは無く、全データが見られればよい。「全部ON」の1操作＝出会ったスキンをスキン表の全スキン、出会った陣形スキルを全レシピ（初出の冒険譚は空＝表示側は初出の行を出さない）、物語の記録をデバッグ以外の全冒険譚・全ステージ（在籍はそのステージまでに登場する actor 全員、イベントはそのステージの全イベント）にする。
