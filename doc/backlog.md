...
ゴールは、その作業で何が達成されていれば終わりなのかを1文で書く。手段ではなく到達点を書く（「タグを決める」ではなく「棚に並んだとき誰の隣に出るかが決まっている」）。作業の途中で軸がずれるのを防ぐために置く。
...
- 作る順序（上から依存している）：
  1. ロゴ。カプセル全種の前提。**作成済み**（2026-08-12。`godot/assets/promo-src/logo/` に暗背景版・明背景版・小サイズ版の SVG、`godot/tools/logo/` に生成スクリプト2本、方針と寸法は [logo.md](art/logo.md)）。PNG への変換は `godot/tools/rasterize_svg.gd`。残るのは用途ごとの書き出し
...
**キービジュアルの画題を決める**（優先度：中）

- 背景：素材の要素と、それぞれが何を伝えるかは [marketing.md](sales/marketing.md) に決めた。ジャンルの識別はロゴ、中身の伝達は紹介画像が担当するので、キービジュアルに残るのは棚で目を留めさせる引きだけ。その1枚に何を描くかが決まっていない。
- 露出先はどれもサムネイル。Steam のカプセル枠（ヘッダー 460×215／小 231×87／メイン 616×353／縦 374×448／ページ背景 1438×810）・itch の cover 630×500・OGP の3つで、公式サイトの本体には席が無い（先頭は扉の絵＝[site.md](sales/site.md)）。一覧に並ぶと 200px 前後まで縮む。
- 対応：サイズの段ごとに何を載せるかを決める。1枚を全枠で共用するのではなく、段によって読めるものが違う。
  - 小（231×87 前後）… ロゴ＋竜。人物は潰れる（下の実測）
  - 中（460×215・616×353・374×448）… 竜と人物が両方読める
  - 大（1438×810 のページ背景）… 主役ではないので情報量の少ない絵
  竜の絵は生成し直さず、ユニットの絵（`godot/assets/units/red_dragon/` の map か combat）を持ってくる。透過済みで輪郭が太く、縮めてもシルエットが残る。
- 実測（2026-08-12。feature-53 のカプセルの検討結果）：手持ちの竜の絵を 460×215 に切ると構図はそのまま生きるが、231×87 まで縮めると読めるのは竜と炎だけで人物は潰れる。ロゴ側も小サイズ版は高さ 87px で読めることが要件（[logo.md](art/logo.md)）。
- 紹介画像はここでは扱わない。並びと狙いは marketing.md に決まっていて、残りは実機を撮る作業＝feature-53 の作る順序に載っている。
- 該当：`godot/assets/promo-src/`・[marketing.md](sales/marketing.md)・[keyvisual.md](art/keyvisual.md)。関連＝feature-53（Steam ストアページ）・feature-51（映像）。
