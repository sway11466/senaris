...
**盤の上で言語を切り替えても情報板の案内文が前の言語のまま残る**
- ゴール：盤の上で言語を切り替えた直後に、情報板に出ているものが何であれ新しい言語で読める。
- 背景：駒が選ばれていないときの案内文（「駒を選んでください」と［操作］の一覧）は `clear()` が描いた文字列をそのまま持っており、`refresh_labels()` は戦闘結果の票しか貼り直さない。駒を選んで解除すれば新しい言語で出るが、切り替えた直後は前の言語のまま。駒や地形を表示している最中の切り替えも同じく貼り直されない。
- 対応：情報板が「いま何を出しているか（案内文／駒／地形／戦闘結果）」を控え、`refresh_labels()` でそれを同じ内容で描き直す。作り直しはしない（[i18n.md](tech/i18n.md) 言語の切り替え）。
- 該当：`godot/presentation/ui/unit_info_panel.gd`（`refresh_labels`・`clear`）・`godot/presentation/main/main.gd`（`_refresh_labels`）。
...
- 該当：`doc/sales/naming_decision_senaris.md`・`doc/sales/site.md`・`site/`。着手の引き金＝配布が見えてきたとき（parking lot「Steam 配布の段取り」と連動）。サイトのランディングページはストアページ（[steam_page.md](sales/steam_page.md)）のあと。
...
- ゴール：情報板を畳んで遊ぶ人にとって、ステージの開始・終了の会話と、ステージが開いた直後の盤の見え方が、畳んでいることと矛盾しない。
- 背景：会話パネルは情報板と同じ矩形に出る別の板なので、畳んでいても intro／outro の会話は右にそのまま出る＝畳んだ人にも板が強制的に出て見える（[uiux.md](gdd/uiux.md) 最小化は「会話パネルは別の板」とだけ書いている）。また、ステージ読み込み時の自動フィットは板を開いた前提の盤エリア（左 800px）に合わせるので、畳んでいるときは右側が空いたまま小さく出る。最小化では「情報板以外は動かない」と決めたが、これは畳む・開くの操作の話で、読み込み時の初期表示までは決めていない。
- 対応：【未決】(1) 畳んでいるときの会話の出し方＝今のまま右に出す／会話の間だけ出して終わったら消える（今の挙動を明記する）／別の場所に出す。(2) 畳んでいるときの読み込み時フィット＝画面幅で合わせるか。畳んだまま開いたときに合わせ直すかは feature-98 の決定（動かさない）を優先する。
- 該当：`godot/presentation/main/main.gd`（会話の置き場・`set_covered`）・`godot/presentation/board/hex_board_3d.gd`（フィットの可視幅）・`godot/presentation/ui/ui_layout.gd`・`doc/gdd/uiux.md`。着手の引き金＝畳んで遊んで気になったとき。
