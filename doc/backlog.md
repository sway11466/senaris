...
- 対応：(1) `data/i18n/units.csv` を新設し、規約キー（`unit.{skin_id}.name`・`terrain.{skin_id}.name`・`movement.{id}.name`）で表示名を解決。`UnitSkin`/`TerrainSkin`/`Movement` の表示名参照を `tr()` 経由に差し替え、データCSVの `name` 列は開発用メモに降格。(2) `data/i18n/ui.csv` を新設し、presentation の直書き文言（`ui.*` キー）を一括キー化。test_i18n_translation の検出範囲に新CSVを加える。(3) 部隊名はステージJSONの `name` をキーに差し替える（表示側は `tr()` を通してあるので置き換えるだけ）。キーの命名と、マップエディタで日本語を打つオーナーの手をどう受けるかをここで決める。
- 該当：`data/i18n/`（units.csv・ui.csv 新規）・`data/units/unit_skin.gd`・`data/terrain/terrain_skin.gd`・`data/movement/movement.gd`・`presentation/ui/`（hud・unit_info_panel ほか）・`data/stages/**`（部隊 `name`）・`tools/map_editor/`（部隊名の入力）・`project.godot`（translation 登録）・`tests/unit/test_i18n_translation.gd`・`doc/tech/i18n.md`。
...
**タイトル画面の残り（ロゴ・クレジット画面）**

- 背景：タイトル画面そのものは入った（起動→扉が開く動画→店内のメニュー。仕様 → [title.md](gdd/title.md)）。残るのは絵の上に載せるタイトルロゴと、メニューに項目だけ置いてあるクレジット画面。
- 木の板の絵：`assets/menu/title_board.png`（元絵とプロンプトは `assets/menu-src/title_board/`）。タイトルのメニュー板として作ったが、操作の道具は酒場の物にしない方針にしたため使っていない。絵としては使えるので、どこで使うかは未決。配線は `tavern_theme.gd` にスロットを1つ足すだけ（消した実装は git に残っている）。
- ロゴ：画面のどこに置くか未決。店内の絵は左をメニュー板が、右をカウンターの主人が塞ぐので、上端の梁の暗い帯が候補。絵に文字を入れないのは全アセット共通のルールなので、UI 側で載せる（吊り看板は作らなかった）。
- クレジット：素材の権利表記。タイトルのメニューに項目は置いてあるが、受け口が無く押せない状態。画面に出す内容は [credits.md](sales/credits.md) の「ゲーム内クレジットに出すもの」が正本で、そこを読んで並べるだけにする。台帳の整備自体は済んでいるが、根拠が取れていないライセンスが残っている（feature-54）。決めていないのは、制作に使った道具（MuseScore・ImageMagick・FFmpeg・Inkscape ほか）を画面にも出すか＝台帳には義務の有無に関わらず載せてあるが、画面に載せる範囲は未決。リリース前が締め切り。
...
**参照側に書き写した性能値を洗い出して消す**（優先度：低）

- 背景：ユニットの性能値の正本は `data/units/unit_type.csv` だが、参照側のドキュメントやコメントに同じ値が書き写されている箇所がある。バランス調整で値を変えると気付かないまま不一致になる。実際に飛空艇の搭載数が doc=6／CSV=4 でずれていた（写しの削除は済み）。判明しているのは冒険譚の駒一覧の性能値列（`0/0/10/1/6` の形式）で、他にも残っている可能性がある。
- 対応：doc と `.gd` のコメントを洗い、CSV の写しになっている数値を消す。消せない場合（読み手に値が要る場合）は正本へのリンクに置き換える。冒険譚の駒一覧は、性能値の列を落として役割の説明だけにするか、列ごと残して不一致を許容するかを決める。仕様側が持つ閾値（[ai.md](gdd/ai.md) の輸送ユニット＝搭載数2以上 など）は写しではないので対象外。
- 該当：`doc/campaign/`・`doc/gdd/`・`domain/`・`data/`。着手の引き金＝ユニットのバランス調整をする回。
