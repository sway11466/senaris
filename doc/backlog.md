...
- 対応（3b-3・セレクトの可視化）：持ち越しの有無は戦い方を変える（主力を死なせられない温存プレイか、使い切ってよいか）ため、挑む前に分かるようにする。ステージ一覧の左に隊ごとの綴じ紐（レーン）を通し、待機区間は薄く、合流するステージで束ねる。判定は `campaign.json` の stage エントリに置く `party`（表示専用・文字列か配列）で、ステージJSONは開かない。ステージJSONを開くと1冒険譚あたり 54〜89ms かかる（実測・7ステージ）＝貼り紙を押すたびに引っかかるため。隊名は画面に出さない（作者用の識別子）。冒険譚カード（上位）には出さない。
