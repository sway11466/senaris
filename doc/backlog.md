...
- 背景：判定・記録・戦果票の表示まで入っている。勝利の印そのものがランク（`S`/`A`/`B`）で、戦果の各行の右に S・A の基準と達成チェックが並ぶ。閾値はステージJSONの `rank` に明示（暗黙知を持たせない）。仕様 → [rank.md](gdd/rank.md)・[uiux.md](gdd/uiux.md)。
- 対応：(1) 閾値の調整＝実際にプレイしながらステージごとに詰める。全21チュートリアルステージに 50%/75% 基準の叩き台が入っている。(2) セレクト画面での見せ方を決める。冒険譚の貼り紙は制覇で `DONE` の印、ステージの木札はクリアで `✓` が付くだけで、どちらもランクを出していない。冒険譚まるごとに1つ出すなら全ステージのランクを束ねる規則が要る。
- 該当：`godot/data/stages/*.json`（閾値）・`godot/presentation/select/campaign_select.gd`・`godot/presentation/select/stage_select.gd`（セレクトでの見せ方）。
