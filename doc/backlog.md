...
次回採番: bug=3 / feature=52 / refactoring=9
...
- 背景：[naming_decision_senaris.md](sales/naming_decision_senaris.md) でタイトル名は「Senaris」に決定済みだが、確定前の手続きが残っている。すべてオーナー側の手作業。Steamコミュニティ `id/senaris` は他者が使用（要再確認）。
...
- サイトの方針：作り込まない。1ページだけ＝キービジュアル・一行の説明・Steamページへのボタン、必要なら併売先（BOOTH／itch）へのリンクとプレスキット（素材の zip と、タイトル／ジャンル／価格／発売日／連絡先の事実関係）。ニュース欄やブログは作らない（更新義務だけが残り、放置されたサイトは逆効果）。ホスティングは静的で無料のもので足りる。中身はストアページが決まってから作る＝載せる文と絵をそのまま流用できる。
