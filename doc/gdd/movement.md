# 移動システム

ユニットの移動範囲（reachable）を、移動タイプ×地形の進入コストで決める。

---

## 設計

### 移動タイプ＝完全表（差分なし）

- 移動タイプは **「地形→進入コスト」の完全な表**。各移動タイプが全地形のコストを明示的に持つ（差分上書きはしない＝定義時の一覧性を優先）。
- **1ユニット＝1移動タイプ**（`UnitType.move_type` → `Unit.move_type`）。エルフのような特殊は、専用の完全表（例 `forest_walk`）を1つ用意して参照する（「徒歩＋森だけ特殊」を1枚の表に書ききる）。
- **`x`＝進入不可**。表に無い地形は既定コスト1（表が空＝全地形1＝従来の一律移動と等価）。

### コスト計算

- `reachable` はダイクストラ（`Hex.flood_reach_cost`）。起点から各ヘックスへの**最短コストが移動力以内**なら到達可能。
- 進入コスト ＝ そのヘックスの地形に対する移動タイプのコスト。盤外・他ユニット占有は進入不可。
- 実装: `data/movement/movement.gd`（`Movement`）／`domain/battle_state.gd`（`reachable`/`_enter_cost`）／`domain/hex/hex.gd`（`flood_reach_cost`）。

### データ

- **配置は独立フォルダ `data/movement/`**。移動コストは「移動タイプ（ユニット属性）×地形（マップ属性）→コスト」の関係（行列）で、units にも terrain にも属さない独立概念のため。
- 正本 `data/movement/movement.csv`（行=移動タイプ、列=地形コスト、2行ヘッダ）→ 生成 `data/movement/movement.json`（[[csv-data-pipeline]] と同じ仕組み）。
- 地形名は `Terrain.NAMES`（`plain`/`plateau`…）と一致させる。
- 現状の移動タイプ（全コストは `movement.csv` が正本＝地形13種 × 7タイプ）:
  - **ground**（地上・基準）／**flight**（飛行・地形をほぼ無視）／**light_foot**（軽歩行・斥候＝山や城壁も越える）
  - **forest_walk**（森を低コスト＝エルフ）／**bush_walk**（茂みを低コスト＝ハーフリング）／**mountain_walk**（山を低コスト＝ドワーフ）／**fixed**（移動0＝バリスタ/バリケード）

- 地形は **データ駆動**（[combat.md](combat.md) 地形効果／`data/terrain/terrain.csv`）。地形を増やす＝terrain.csv に1行＋この movement.csv に1列。山・水辺なども同様。

---

## ZOC 停止（実装済み）

- **敵ZOC（敵ユニットに隣接するマス）に入ると、そこで移動終了**＝その先へは進めない。マス自体には入れる。
- **飛行を含む全移動タイプに適用**（移動タイプ非依存）。包囲（戦闘補正）とは別軸。
- **起点が敵ZOC内でも動き出せる**（起点には停止判定を適用しない）。
- 実装: `BattleState.reachable`/`_in_enemy_zoc`、`Hex.flood_reach_cost` の `stop_fn`（終端ヘックス＝到達可だが非展開）。

## 攻撃後の再移動（ヒット&アウェイ・実装済み）

- ユニットデータ `move_after_attack`（CSV列／`UnitType`／`Unit`）が true のユニットは、**攻撃後に残り移動力で再移動**できる（ネクタリスのバギー＝シーフ/ラビット系）。
- **移動予算は move を1ターンで共有**：攻撃前の移動＋攻撃後の再移動の合計コストが `move` 以内。`_spent` で消費を追跡。
- 行動単位は「攻撃前の移動1回／攻撃後の再移動1回（再移動可ユニットのみ）」。**ZOC停止と両立**：移動は1回ぶんが ZOC で止まるので、止まったマスから再移動でZOCをすり抜けることはできない（再移動は攻撃後の別枠）。
- 実装: `BattleState`（`_spent`/`_moved`/`_post_moved`、`move_unit`/`_can_act_move`/`can_still_move`/`is_done`）。

## 輸送（積載・運搬・実装済み）

ユニットを載せて運ぶ仕組み（ネクタリス同等）。輸送ユニット＝`capacity ≥ 1`（[units.md](units.md)。馬車4・飛空艇6・他0）。

- **乗る（board）**：ユニットを**輸送の駒に重ねる**（輸送の hex へ移動する）と乗る。乗った駒は**行動完了**になる。輸送の `capacity` に空きがある場合のみ乗れる。**輸送のマスは移動の終点としてのみ進入可**＝すり抜けて先へは行けない。
- **運ぶ**：輸送ユニットは中の駒を載せたまま**通常移動**する。乗った駒が完了でも**輸送は別ユニットなので同一ターンに動かせる**（乗せて即運搬が可）。
- **降りる（unload）**：中の駒が**未行動なら、その駒は通常通り行動できる**（＝輸送の位置を起点に**通常の移動＋攻撃**。移動だけに制限しない）。降車＝その駒の移動としてコストを消費し、占領可ユニットが拠点hexへ降りれば**即占領**（移動と同じ扱い）。**降車は搭乗駒の行動**＝輸送自身が行動完了（移動・攻撃・待機済み）でも、未行動の駒はいつでも降ろせる（輸送を選択→降車メニュー）。
- **降車先**：その駒が**進入できる空きhex**（その駒の `move_type` の地形コストが `x` でないマス）。例＝**空中ユニットは崖の上にも降りられる／地上ユニットは崖・城壁・壁には降りられない**。→ 飛空艇で地上駒を運んでも、降ろせるのは地上が入れるマスだけ。
- **隣接1マスの特例（未実装）**：乗車・降車は**輸送に隣接するマスとの間なら、移動力・地形コストに関係なく行える**（荷の積み降ろしは人手で行うイメージ）。移動力の数値は変えない（盤上を歩けるようにはならない）。
  - 効くのはコスト計算だと乗降できないケースだけ：移動0のバリケード（馬車が隣に来て拾う／隣接マスへ降ろす）、移動1の駒が高コスト地形の上の輸送に乗る 等。移動力で届く範囲の乗降は現行どおり。
  - 副次効果＝置き間違えたバリケードの回収が可能（馬車を隣に寄せて積み直す）。
- **運べる対象**：輸送の所有者と**同陣営の駒**（味方が味方を運ぶ）。**輸送は輸送に乗れない**。占領の `native`／拠点とは別軸。
- **輸送の撃破**：搭乗中の駒も**巻き添えで失われる**（撃破扱い＝ボス撃破条件にも数える）。
- **輸送自身の戦闘**：データ駆動（CSVの攻撃値どおり＝馬車10なら弱いが殴れる）。
- 実装: 搭乗リストは `BattleState._passengers`（盤上に居ない＝殲滅カウント外）。乗車＝`move_unit` が輸送hexへの移動を搭乗に変換。降車＝`unload_cells`/`unload`＋`UnloadCommand`。初期搭乗＝ステージJSONのユニットに `"passengers": [...]`（st7の「飛空艇にパラディン」用）。UI＝乗車メニュー・輸送のコマンドメニューに「降車: 〈駒〉」（乗車したターンは無効表示）・搭載数バッジ。敵AIは乗降しない（当面）。

## 未決・残作業

- [ ] 地形のデータ化（移動コスト列の拡張＝森・山・水辺…）と、地形タイプごとのコスト確定。
- [ ] 移動タイプの拡充（騎乗・水棲・特殊パターン）。
- [x] 輸送の実装（搭乗状態・board/unload・capacity 配線）。輸送自身の戦闘＝データ駆動（CSVの攻撃値どおり）で確定。検証: `data/stages/debug/transport.json`。
- [ ] 敵AIの乗降（輸送を使う敵）。
- [ ] 隣接1マスの特例（上記）の実装：乗車＝`reachable` に隣接する乗車可能な輸送のマスを常に含める／降車＝`_unload_map` に隣接の進入可能マスを合流。

## 関連ドキュメント

- [combat.md](combat.md) — 戦闘解決・地形効果
- [map.md](map.md) — 地形・拠点
- [units.md](units.md) — ユニット（move_type）
