...
- **type の名前も flavor-neutral**：`id` は味方に居てもおかしくない名前にする。性能は全スキンで共有されるので、特定の魔獣の名前を型に付けると他のスキンから流用しづらくなる（ハーピーの性能を味方の軽飛行に貸したいとき、型名が `harpy` だと据わりが悪い）。魔獣の名前はスキン側に置く＝型 `harrier`／スキン `harpy`、型 `serpent` 相当の `scout`／スキン `rock_serpent`。`dragon` のように味方側にも成立しうる名前は例外的に許容する。
