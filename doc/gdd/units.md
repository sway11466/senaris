...
- **行動回数（`actions`）と攻撃チェンジ（副武装）**＝斥候の攻撃後移動と同じ並びの、type の固有能力。
  - `actions`（行動回数。既定1）: 1手番に実行できるコマンドの数。移動は最初の行動の前に1回だけ（攻撃後移動を持つ型は、その分は別）。行動は「攻撃」か「攻撃チェンジ」。攻撃は1回ごとに通常の戦闘（距離1なら反撃あり・包囲や支援の補正も毎回乗る）。同じ相手でも別の相手でもよい。
  - **副武装**（`alt_atk_ground`／`alt_atk_air`／`alt_pierce`／`alt_range`。空なら武装1組＝チェンジ不可）: 型が2組目の攻撃を持てる。「攻撃チェンジ」は行動を1つ使って主武装⇄副武装を切り替える。切り替えた状態は次の手番にも続き、反撃も現在の武装で行う。
  - 使い方の基本形は **行動3＝攻撃 → 攻撃チェンジ → 攻撃**（近接で殴り、術に持ち替えて撃つ）。古代遺跡 第1部のボスが初出（[../campaign/ancientruins1-mine-monsters.md](../campaign/ancientruins1-mine-monsters.md) st7）。当面は敵の精鋭にだけ載せ、味方には持たせない。
  - 敵AIは行動ごとに狙いを別々に決められるようにする（[ai.md](ai.md)）。
- **精鋭（`champion`）はシールド8を標準にする**（ドラゴンだけ16）。シールドは兵数の手前で損害を受ける器（[combat.md](combat.md) シールド）。ボス級に「あと何発で剥がれるか」を読ませるための値で、新しい精鋭の型を足すときも 8 から始める。
...
- `capacity`（搭乗数）＝**輸送できる"ユニット数"**。`capacity ≥ 1` なら輸送ユニット（別途フラグは持たない）／非輸送＝0。乗降＝乗せる/降ろす操作・移動消費・降車後の行動可否は `BattleState`（`can_board`/`put_passenger`/`unload`）・`godot/application/commands/unload_command.gd`。テスト `godot/tests/small/domain/test_transport.gd`。敵AIの乗降は [ai.md](ai.md)（輸送ユニット）。
- `shield`（シールド）＝兵数の手前で損害を受ける器の初期値。無い種別は 0。減り方・回復しないこと・表示は [combat.md](combat.md) のシールド。
- `actions`（行動回数・既定1）と副武装 `alt_*`（攻撃チェンジ）＝上記 性能設計方針。列はまだ CSV に無い（初出のボスを実装するときに足す）。
