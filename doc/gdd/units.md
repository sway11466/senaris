...
- **攻撃の種別（3軸）**：①**対地/対空**（飛行相手は `atk_air`、`atk_air=0`＝対空不可で攻撃対象に選べず反撃もしない）②**貫通率 `pierce`**（相手防御を無視する割合。魔法兵0.5＝防御半減／物理0＝対アーマー）③**射程 `range`**（≥2＝間接・反撃なし）。**対空の担い手＝弓兵・飛行・魔法兵**、占領兵・歩兵・斥候の近接は対空なし（＝「近接が空を殴る」矛盾を避ける）。**魔法兵＝対アーマー／弓兵＝対空・機動**で住み分け。貫通の戦闘適用は [combat.md](combat.md)。
