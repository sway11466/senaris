...
- `capacity`（搭乗数）＝**輸送できる"ユニット数"**。`capacity ≥ 1` なら輸送ユニット（別途フラグは持たない）／非輸送＝0。現状: **馬車(wagon)=4・飛空艇(airship)=6**、他=0。**データ正本に定義済み・コード未配線**（積み下ろし＝乗降の細則＝乗せる/降ろす操作・移動消費・降車後の行動可否 等は輸送メカ本体として別途設計）。
