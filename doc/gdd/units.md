...
- `capacity`（搭乗数）＝**輸送できる"ユニット数"**。`capacity ≥ 1` なら輸送ユニット（別途フラグは持たない）／非輸送＝0。現状: **馬車(wagon)=4・飛空艇(airship)=4**、他=0。**実装済み**（乗降＝乗せる/降ろす操作・移動消費・降車後の行動可否は `BattleState`（`can_board`/`put_passenger`/`unload`）・`application/commands/unload_command.gd` に配線済み。テスト `tests/unit/test_transport.gd`。ただし敵AIの乗降は未対応）。
