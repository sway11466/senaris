...
| **クリア済み（cleared）** | 札の右端に評価ランク（[rank.md](rank.md)）の印を焼き印の色で押す。ランクを持たないステージだけクリアバッジ（✓）。**常に選択可**＝再挑戦できる |

### 幕間の印（ステージとステージのあいだ）

継承（carryover）の冒険譚では、話と話のあいだに何が起きるか（兵が戻るのか、戻らないのか）がそのまま難しさになる。リストの行と行のあいだに小さな印を挟んで、遊ぶ前に読めるようにする。印はマニフェストの `interlude`（[冒険譚マニフェスト](#冒険譚マニフェスト)）から引く。

| `interlude` | 印 | 意味 |
|---|---|---|
| `rest` | ベッド（小） | 休息。兵が満ちる（次の話の名簿の駒に `supply: "refill"`） |
| `revive` | 十字（小） | 復帰。倒れた仲間も戻り、兵が満ちる（`supply: "revive"`） |
| 未指定 | なし | 連戦（兵は戻らない）。独立（各話配給）の冒険譚もここ |

- 印は「次の行の前」に付く＝st2 の前に休息があるなら、st1 と st2 のあいだにベッドが出る。
- 未解放の行の前でも印は出す（伏せるのは題名だけ）。
- 連戦に印は出さない＝ベッドが無いことで「休んでいない」と読ませる。
- 絵は小さなアイコン2つ（[../art/icons.md](../art/icons.md) 幕間の印）。復帰の十字は依頼書の復帰の印と同じ形＝同じ意味に同じ形。

---

## 依頼書

ステージの札を押すと出る羊皮紙のダイアログ。出撃確認のワンクッションであり、知らずに挑むと理不尽になることだけを書く。
...
- **兵がどう用意されるかは駒ごとの印**で言う。駒の絵の下に、印の絵と語を並べて置く（絵は [../art/icons.md](../art/icons.md) 依頼書の印）。絵に重ねると顔か足元のどちらかを潰すので、重ねずに下へ出す。意味は語が言い、絵は目印＝語が読めれば絵が無くても伝わる。同じ群に事情の違う駒が混ざるので、群では言い分けられない。
  - 「新加入」＝初登場（`supply: "join"`）／「復帰」＝兵数ゼロから満員で戻る（`revive`）／「補充」＝損耗した兵数が満員へ戻る（`refill`・`revive`）／「損耗」＝損耗したまま出る（`supply` 省略）。供給の指定は [campaigns.md](campaigns.md) 配置。
  - 満員のまま持ち越す駒は印を出さない＝補充しても変わらないものに印を付けない。`refill` が付いていても名簿が満員なら印は出ない。
  - この依頼限りの駒（`actor` を持たない駒・輸送の乗員）は印を持たない。引き継がないことは群の見出しが言う。独立のステージは全員がこれに当たる＝印の出ない紙になる。
  - 兵数ゼロで出撃できない駒も印を持たない。出られないことは沈めた色が言う。
  - 駒の真下に来るのは**語の中心**で、印はその左へ張り出す。小さい印と長い語をまとめて中央に置くと、目には語が右へ寄って見える。
  - 印が出る群は、駒を並べる升の幅を固定する。語の長さで決めると、駒の間隔が語の長さで変わり、言語を変えただけで並びが動く。
- 勝利条件・推奨戦力は載せない＝盤とステージ開始の会話で読ませる（先に文字で説明すると導入の会話と重複する）。
- 未解放の札を押したときは同じ紙に解放条件だけを出す。顔ぶれも供給も出さない（ステージ名を伏せている紙が中身を漏らさない）。
- 紙の寸法は変えない（材質テクスチャと同寸＝[../art/menu.md](../art/menu.md)）。顔ぶれは紙の幅で折り返す。
...
- `interlude`（任意・ステージ項目）: その話の前の幕間＝`rest`（休息）／`revive`（復帰）。連戦は書かない。見せ方専用の印で、兵が実際にどう戻るかはステージ JSON の `supply`（[campaigns.md](campaigns.md) 配置）が決める。`rest` の話は新入り（`join`）以外の名簿の駒がすべて `refill`、`revive` の話はすべて `revive`、印の無い話には `refill`／`revive` の駒が無いはずで、食い違いはデータ整合テストで検出する（1面の前に幕間は無い。デバッグ冒険譚は対象外）。上記以外の値は警告して印を出さない。ステージ一覧の[幕間の印](#幕間の印ステージとステージのあいだ)がこれを見る。連続プレイの途中では何も挟まない＝休息の様子は各冒険譚の戦闘前会話が言葉で伝える。
...
- クリア評価（ランク S/A/B）も同じ場所に記録し、ベストだけを残す（仕様 → [rank.md](rank.md)）。ステージ一覧はこれを札の印として読む。冒険譚単位の Steam 実績「全ステージを上位ランク以上で踏破」が全ステージのランクを参照する（[../sales/monetization.md](../sales/monetization.md) 実績・計測）。
