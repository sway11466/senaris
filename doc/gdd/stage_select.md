...
- レーンは隊ごとに1本。隊を分ける冒険譚（[map.md](map.md) 隊を分ける）では線が並走する。
- 出番のないステージの区間は薄い線にする＝その隊が待機していて、あとで戻ってくることが読める。
- 合流するステージでレーンを束ねる。
- 隊の名前は出さない。`party` は作者用の識別子で、プレイヤーに見せる語ではない。線の並びと束ね方だけで伝える。
...
ステージの `party`（任意）は、そのステージがどの隊で戦うかの覚え書き。[連戦の区間](#連戦の区間)の線を引くためだけに使い、ゲームの動きには関与しない（実際の編成は `actor` と `carryover_slots` が決める → [map.md](map.md) 隊を分ける）。

```json
{ "id": "st5", "file": "st5.json", "title": "t3.st5.title", "party": ["A", "B"] }
```

- 値は作者が決める識別子。1つの隊なら文字列（`"party": "A"`）、合流するステージでは配列で並べる。
