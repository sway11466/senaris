...
- 満たさない場合はステージセレクトへ戻る。
- **幕間の挿絵**: 自動遷移するとき、次のステージの `interlude` が `rest` か `revive` なら、次の戦闘前会話の前に一枚絵を挟む（ベッドの挿絵＋「一晩休んだ。兵が戻った。」／合流の挿絵＋「離れていた仲間が戻った。」）。クリックかキーで進む。`continuous` と未指定は何も挟まない＝連戦はそのまま次の会話へ。挿絵は2枚（[../art/keyvisual.md](../art/keyvisual.md)）、文は翻訳キー。セレクトから直接そのステージを選んで始めたときは挟まない（印がリストに出ているため）。
...
| `interlude` | 印 | 意味 |
|---|---|---|
| `continuous` | 松明（小） | 連戦。兵は戻らない |
| `rest` | ベッド（小） | 休息。兵が満ちる（次の話の名簿の駒に `supply: "refill"`） |
| `revive` | 合流の旗（小） | 復帰。離脱した仲間も戻り、兵が満ちる（`supply: "revive"`） |
| 未指定 | なし | 独立（各話配給）の冒険譚、または印を出さない |

- 印は「次の行の前」に付く＝st2 の前に休息があるなら、st1 と st2 のあいだにベッドが出る。
- 未解放の行の前でも印は出す（伏せるのは題名だけ）。
- 絵は小さなアイコン3つ（[../art/icons.md](../art/icons.md)）。
