...
- 戻りは常に左下。ステージ一覧の「← 冒険譚」で冒険譚選択へ、冒険譚選択の「← メニュー」でタイトルのメニュー（[title.md](title.md)）へ、一段ずつ戻る。冒険譚選択では Esc も同じ入口を通る。上端は名前（ステージ一覧＝冒険譚名／冒険譚選択＝ボード名）の場所なので空けておく。戻りのボタンは繰り矢印・現在地ドットと同じ無機質なグレーで、酒場の物にはしない。
