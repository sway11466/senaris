...
- クリア評価（ランク S/A/B）も同じ場所に記録する。判定・記録の仕組みは実装済み（仕様 → [rank.md](rank.md)）。冒険譚単位の Steam 実績「全ステージを上位ランク以上で踏破」が全ステージのランクを参照する（[../sales/monetization.md](../sales/monetization.md) 実績・計測）。
