...
- **兵がどう用意されるかは駒ごとの印**（バッジ）で言う。絵の右下に重ねる（絵は [../art/icons.md](../art/icons.md) 依頼書の印）。上に置くと顔に掛かる＝誰が出るのかを顔で読む紙なので、隠すなら足元にする。同じ群に事情の違う駒が混ざるので、群では言い分けられない。
  - `New!`＝初登場（`supply: "join"`）／`Revive`＝兵数ゼロから満員で戻る（`revive`）／`Refill`＝損耗した兵数が満員へ戻る（`refill`・`revive`）／`Damaged`＝損耗したまま出る（`supply` 省略）。供給の指定は [campaigns.md](campaigns.md) 配置。
