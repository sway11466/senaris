...
- 戻りは常に画面の左下、同じ大きさの木の板ボタン。ステージ一覧の「← 冒険譚」で冒険譚選択へ、冒険譚選択の「← メニュー」でタイトルのメニュー（[title.md](title.md)）へ、一段ずつ戻る。冒険譚選択では Esc も同じ入口を通る。上端は名前（ステージ一覧＝冒険譚名／冒険譚選択＝ボード名）の場所なので空けておく。背景の絵に合わせて位置を変えない＝画面が変わっても戻るは同じ場所にある。
