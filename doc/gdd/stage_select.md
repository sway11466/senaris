...
戦力を持ち越す連戦（[map.md](map.md) 戦力供給モデル）は、挑む前に分かるようにする。持ち越しがあるかどうかで戦い方が変わる（主力を死なせられない温存プレイか、使い切ってよいか）ので、知らずに初戦で主力を失う理不尽を避ける。

- 同じ戦力を持ち回るステージを、リストの左に通した綴じ紐（レーン）でつなぐ。どこから始まりどこまで続くかが縦の線で読める。
- レーンは隊ごとに1本。その隊が出るステージでは実線、出ないステージでは薄い線（初めて出る話から最後に出る話までのあいだだけ引く）。
- 線が2本とも実線で並走する＝2隊が同じ盤で協力している。片方だけ実線＝もう一方は待機していて、あとで戻ってくる（[map.md](map.md) 隊を分ける）。
- 隊の名前は出さない。`party` は作者用の識別子で、プレイヤーに見せる語ではない。線の本数と続き方だけで伝える。

区間の判定に使うのはマニフェストの `party`（後述）。ステージJSONは開かない＝一覧を出すたびに全ステージを読むコストを持たない。
...
- 顔ぶれは名簿（[campaigns.md](campaigns.md) 継承）と突き合わせた結果＝盤に出るのと同じ並び。兵力ゼロで出撃できない駒も並べ、盤の行動終了と同じ暗さで沈める＝隊に居たことは残り、出られないことは色で分かる。
...
- 顔ぶれは群に分け、群ごとに絵の上へ一行の見出しを置く。見出しが戦力の供給（[campaigns.md](campaigns.md) 独立／継承）を兼ねる＝供給を語る別の一行は持たない。継承かどうかは自軍の駒が `actor` を持つか（名簿に載る駒があれば継承）で決める。
  ...
  - 継承のステージは**2つの群に分ける**＝「前のマップで生存しているユニットが出撃できます」と「このマップ限りで出撃します」。同じステージに両方が居る（連れて行く隊＋その場の馬車やバリケード）ので、混ぜて並べると全部が引き継がれるように読める。居ない群は見出しごと出さない。
...
ステージの `party`（任意）は、そのステージにどの隊が出るかの覚え書き。[連戦の区間](#連戦の区間)の線を引くためだけに使い、ゲームの動きには関与しない（実際の編成は `player` に書いた `actor` が決める → [map.md](map.md) 隊を分ける）。

```json
{ "id": "st5", "file": "st5.json", "title": "t3.st5.title", "party": ["A", "B"] }
```

- 値は作者が決める識別子。1隊なら文字列（`"party": "A"`）、複数の隊が出るステージでは配列で並べる。
- 省略＝どの隊にも属さない単発ステージ。線を出さない。
- 実際の編成とずれても壊れない（線の見た目が変わるだけ）。ずれたら直す。
...
- デバッグ用マップは機能別に6つの **デバッグ冒険譚** へ分ける（`godot/data/stages/debug-combat` / `-ai` / `-victory` / `-mapops` / `-skins` / `-misc`）。各フォルダに `campaign.json`（`"debug": true`）を置く。カテゴリ別の内訳 → [../tech/debug-stages.md](../tech/debug-stages.md)。
