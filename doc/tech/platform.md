...
どの実績を置くかの方針は [../sales/monetization.md](../sales/monetization.md) 実績の粒度＝冒険譚ごとに3段。

### ID

実績の ID は `<冒険譚の実績キー>_<段>`。

| 段 | ID | 条件 |
|---|---|---|
| 踏破 | `<キー>_CLEAR` | 冒険譚の全ステージをクリアした |
| 上位ランク | `<キー>_RANK_A` | 全ステージのベストランクが A 以上 |
| 最上位ランク | `<キー>_RANK_S` | 全ステージのベストランクが S |

- 冒険譚の実績キーはマニフェストの `achievement` に書く（[../gdd/stage_select.md](../gdd/stage_select.md) 冒険譚マニフェスト）。大文字の英字・数字・`_` だけで作る（例 `TUTORIAL1`）。
- 冒険譚 ID から作らない。冒険譚 ID は改名がありうる（[gamesystem.md](gamesystem.md) 版と移行）が、実績の ID はリリース後に変えられない（Steamworks で消すと解除済みの記録ごと消える）。変えられない名前は、変えうる名前から切り離して持つ。
- 題名の部分（`goblin-raid` など）はキーに入れない。仮題のまま進むものがあり、題名が変わってもキーは残る。
- `achievement` を書かない冒険譚には実績が無い（デバッグ冒険譚・収録前の冒険譚）。
- ステージを足している途中の冒険譚にも書かない。全ステージが揃ってから書く。書いた時点のステージで踏破が立つと、あとからステージを足しても解除済みの実績は取り消せない。
- `achievement` を書く冒険譚は全ステージに `rank` を書く（[../gdd/rank.md](../gdd/rank.md)）。ランクを持たないステージがあると、ランクの2段が取れなくなる。データ整合テストで検出する。

### 解除のしかた

実績は進捗セーブ（クリアとベストランク）から判定する。解除したという記録は別に持つ（上の「実績ファイル」）が、条件は進捗だけを見る。

- ステージをクリアして進捗を書いたあと、その冒険譚を判定し直す。条件を満たす段を `unlock` する。
- 最上位ランクを満たしたら、下の2段も同時に `unlock` する（取りこぼし防止。[../sales/monetization.md](../sales/monetization.md) 実績の粒度）。段ごとに条件を独立して見れば自然にそうなる＝S は A 以上でもあり、全ステージのランク記録はクリア記録でもある。
- 起動時にも全冒険譚を判定し直す。実績の仕組みが入る前に進めた進捗（itch の体験版で遊んだぶんなど）も、次の起動で拾える。`unlock` は何度呼んでもよいので、判定済みかは覚えない。

## Stats の中身

計測したいのは、最初の1時間でどのステージで離脱したか（[../sales/monetization.md](../sales/monetization.md) 計測）。

- 対象はチュートリアル（マニフェストの `board` が `tutorial`）の冒険譚だけ。
- ステージごとに2つ数える。始めた回数 `<キー>_ST<n>_START` と、クリアした回数 `<キー>_ST<n>_CLEAR`。`<キー>` は実績と同じ冒険譚の実績キー、`<n>` はマニフェストでの並び（1始まり）。
- 同じステージを遊び直しても、始めた・クリアしたのたびに数える。見たいのは人数ではなく、どこで止まるかの比なので、やり直しを含めた回数で足りる。
- 中断セーブから再開したときは始めたに数えない。同じ挑戦を2回数えることになる。
