...
| `manual/shot_info_panel` | マニュアル用スクリーンショット。情報板だけを実機と同じ幅で撮る。中身は駒の情報（能力／状態／地形）か戦闘レポート（サマリー／攻撃／反撃）で、レポートは実機と同じ経路で1回戦わせた結果を出す。縦は中身の下で切る。板の文字は翻訳されるので言語を必ず渡す | `godot --path godot res://tools/manual/shot_info_panel.tscn -- <出力PNG> <ステージjsonのres://パス> (--select <col,row> ｜ --attacker <col,row> --target <col,row>) --tab <タブ> --locale <ja｜en>` | [../gdd/manual.md](../gdd/manual.md) |
