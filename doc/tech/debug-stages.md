...
| ステージ | ファイル | 状態 |
| --- | --- | --- |
| ユニットスキル（妖精の粉） | `debug-combat/skill.json` | 実装済み |

ユニットスキルの盤は全面平地。ピクシー2体と、掛ける相手（ファイター2・アーチャー・ナイト）を左に固め、索敵0の待機ユニットを右に置いた。自分・隣接の味方・離れた味方で対象の絞り込みを確かめ、そのまま殴って戦闘レポートの加算バフ表示（`妖精の粉 +80/+80`）まで見られる。2体のピクシーで同じ相手に重ねがけもできる。
