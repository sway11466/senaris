...
| dragon_breath | `debug-unit-skill/dragon_breath.json` | ドラゴンブレス（敵）。竜は待ち伏せ・視線1で上端に眠り、下向きの扇の8ヘクスに味方7体が並ぶ。3ターン待機してチャージを溜め、4ターン目に下のナイトを竜の真下へ入れて起こすと、その敵ターンに吐く（カットイン→火の壁が3段で広がる） | ドラゴンブレスを変えたとき |
...
| move | `debug-map/move.json` | 移動（移動タイプ・地形コスト・移動音）。地面・軽い足音・羽ばたき・浮遊・プロペラ・蹄（馬）・車輪と蹄（馬車）の味方を1体ずつ置き、森・墓標・柵・墓石の山の帯を横切らせる。敵はゴーストとレイスの突撃＝敵の手番に飛んで来る浮遊音を実戦の文脈で聞く | 移動タイプ・地形コスト・移動音を変えたとき |
