...
- **たたき台あり（11曲・`.musicxml`／`.mid`）**: `journey` ／ `raid` ／ `dungeon` ／ `forest`（旧 crisis v1 の転用）／ `ruins` ／ `graveyard` ／ `temple` ／ `ritual` ／ `boss` ／ `boss2` ／ `crisis`（v2・警報型）＝各32小節。いずれも MuseScore での仕上げ（強弱・味付け・ループ点整備）と `.ogg` 化は今後。ライブラリ表の未着手は `title` のみ（menu と共用するかは検討中）。
- 曲を追加したら本ドキュメントのライブラリ表と権利台帳を更新する。
- 台帳の実体は **`assets/bgm-src/credits.md`**。
- **メニュー曲**（`menu`・旧マップ曲v3の転用）: 投入済み（76.800秒ちょうど・ループ・ラップアラウンド加算あり）。継ぎ目・曲の頭とも実聴で違和感なしを確認済み。他の曲が揃っていない間は無音＋ログ1行で進む（autowire）。
- **スティンガー**（`victory` 10.3秒 ／ `defeat` 14.4秒）: 投入済み（ループなし）。`main._on_battle_finished` が決着時に `BgmPlayer.play_stinger` で一発再生（ステージ曲を素早く下げ、フェードインなしで頭から出す）。
