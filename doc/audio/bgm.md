...
割当（チュートリアル1）: st1–2 = `journey` ／ st3–5 = `raid`（ここまでステージJSONに記載済み）／ st6–7 = `dungeon` を予定（曲が未投入のため未記載＝現状は無音）。
...
- **たたき台あり（9曲・`.musicxml`／`.mid`）**: `dungeon` ／ `forest`（旧 crisis v1 の転用）／ `ruins` ／ `graveyard` ／ `temple` ／ `ritual` ／ `boss` ／ `boss2` ／ `crisis`（v2・警報型）＝各32小節。いずれも MuseScore での仕上げ（強弱・味付け・ループ点整備）と `.ogg` 化は今後。ライブラリ表の未着手は `title` のみ（menu と共用するかは検討中）。
- 曲を追加したら本ドキュメントのライブラリ表と権利台帳を更新する。
- 台帳の実体は **`assets/bgm-src/credits.md`**。
- **投入済み（5曲）**: `menu`（76.800秒）／ `journey`（71.111秒）／ `raid`（60.952秒）＝いずれもループ長ちょうど・ラップアラウンド加算あり。`victory`（10.3秒）／ `defeat`（14.4秒）＝スティンガー（ループなし）。曲が未配置のステージは無音＋ログ1行で進む（autowire）。
...
- **全体既定は `map_calm`**（`BgmDirector.DEFAULT_STAGE_TRACK`）で、この ID の曲は無い＝ステージにも冒険譚にも `bgm` 指定が無いと無音になる。チュートリアル1の st6–7 が現在これに当たる。既定を投入済みの曲に変えるか、`campaign.json` に既定を書くかは要判断。
