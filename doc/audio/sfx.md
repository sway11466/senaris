...
陣形スキルの音はレシピごとに分ける。面殲滅（トリニティスペル）と全体バフ（ホーリーアリア）と単体狙撃（ディバインジャッジメント）は起きることが違うので、同じ音を鳴らすと何が起きたのかが音から分からない。ユニットスキル（`map_skill`）は分けない＝毎ターン飛ぶため1つで足りる。
...
ユニットスキルは分けない。`map_skill` の1つを共用する。単独で撃てて毎ターン飛ぶため、レシピごとに作っても聞き分ける機会がないまま音数だけが増える。
