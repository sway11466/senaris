...
ステージJSONの `backdrop` に絵のIDを1行書く。靄の濃さ `haze`（水平線での濃さ 0〜1・全ステージ必須）は絵に合わせて選ぶ。
...
`backdrop` を書かなければ水平線を引かず、地面が窓の上端まで続く（今までの見え方）。絵を置けば出る＝コードもCSVも触らない。

靄は背景には掛からず、水平線から下の地面だけを奥ほど暗くする。空の下（屋外）は 0.80 で奥行きが出る。洞窟の壁は地面と色が近いので、0.80 だと水平線直下だけが暗い帯になって壁と床が段に見える＝0.30 に下げる。
