...
| id | 物 | 外形の型 | 意味 |
|---|---|---|---|
| `onward` | 戦の角笛（太い三日月が左下から右上へ・吹き口と真鍮の縁） | 斜めの塊 | 連戦。兵は戻らない |
...
- 外形の型を3つで散らす（斜めの塊・横長・縦長の対称）＝色が近くても取り違えない。`revive` だけは物ではなく徽章＝「復活」に対応する日常の物が無いので、ジャンルの慣習（翼＝蘇生）に乗る。地に立つ石の十字にすると墓標に読まれるので、手のひらに載る金の徽章として描く。
- `onward` は壊れた道具にしない。物の破損は「傷ついた」までしか言わず、連戦（休まず次へ出る）にならない。角笛は出撃の合図＝次の戦いがすぐ来ることを指す。酒を飲む角杯に読まれると休息の側に転ぶので、吹き口の金具と吊り帯を描く。
