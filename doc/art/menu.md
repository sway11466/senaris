...
| `parchment_sheet` | 依頼書（出発の確認ダイアログの紙） | ナインパッチ（縁固定・**縦も横も引き伸ばし**）。実寸 560×400 に対し、紙は見開きの2段組で横長に使う（→ [../gdd/stage_select.md](../gdd/stage_select.md) 紙の作り）。タイルにすると元絵の幅・高さのところで繰り返しの継ぎ目が出て、折り目のような筋に見える | 8px | クリーム地＋薄縁＋落ち影 |
...
エンジンが最初のシーンより前に出す静止画。ロゴ（暗背景版）の右下に開発元名 `craftkobo` を小さく添えた1枚で、絵は足さない。ロゴの方針は [logo.md](logo.md) が正本。
