...
起動時に酒場の扉が開いて店内へ入る動画を流し（スキップ可）、入り終わりの店内静止画の上にタイトルメニューを出す（[backlog.md](../backlog.md) feature-46）。セレクト画面の依頼ボードは「入って振り向いた先」にあたる＝扉からは見えない向きの壁なので、店内の映像とボードの画は矛盾しない。
