...
| `parchment_sheet` | 依頼書（出撃確認ダイアログの紙） | ナインパッチ（縁固定・**縦も横も引き伸ばし**）。実寸 560×400 に対し、紙は見開きの2段組で横長に使う（→ [../gdd/stage_select.md](../gdd/stage_select.md) 紙の作り）。タイルにすると元絵の幅・高さのところで繰り返しの継ぎ目が出て、折り目のような筋に見える | 8px | クリーム地＋薄縁＋落ち影 |
