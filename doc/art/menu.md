...
| `parchment_sheet` | 依頼書（出撃確認ダイアログの紙） | ナインパッチ（縁固定・縦は引き伸ばし／横は実寸と同じで1:1）。実寸 560×400＝`QuestSheet.SHEET_SIZE`＝紙の最小寸法で、顔ぶれの群が増えると縦に伸びる（タイルだと繰り返しの継ぎ目が出る） | 8px | クリーム地＋薄縁＋落ち影 |
