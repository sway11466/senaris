...
| `title_board` | タイトルのメニュー板（[../gdd/title.md](../gdd/title.md)） | 1枚絵を丸ごと伸縮（ナインパッチにしない）。実寸 768×1376 | — | 暗い木の看板（`signboard_stylebox`） |
...
- `title_board` だけはナインパッチにしない。枠のレールと四隅の鉄具が1枚に描き込まれていて、辺を引き伸ばすと飾りが崩れるため、絵を丸ごと伸縮させる。歪ませないよう、置く側が絵の縦横比を保つ枠に収める。飾りを描き込む代わりに縦横比を固定した、という交換だと考える。
