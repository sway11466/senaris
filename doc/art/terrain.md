...
- 【視覚】地形は盤上で標高を持てる（見た目のみ・性能不変）。高さは [terrain_skin.csv](../../data/terrain/terrain_skin.csv) の `elevation` 列＝地形タイプではなくスキンごとに持ち、[hex_board_3d.gd](../../presentation/board/hex_board_3d.gd) がその値でタイルを持ち上げ、低い隣接辺／盤外辺に崖のスカートを下ろしてメサに見せる。現状の値（TILE=1.0＝中心〜頂点 を単位）は 壁0.70／崖0.36／瓦礫の山0.36／台地0.18／街区0.18／森0.18／茂み0.12。ユニット・影・グリッド・拠点・オーバーレイ・クリック判定（ピッキング）も標高に追従する。
- 【視覚】高さが「地形の厚み」なら、駒がその上に立つか中に立つかは `terrain_skin.csv` の `sprite_sink`（立ち絵の沈み）が決める。よじ登る地形（城壁・瓦礫の山）は沈み0＝上面に立つ。木々や建物の間の地面に居るべき地形（森・茂み・街区）は高さと同じ値だけ沈める＝森0.18/0.18・茂み0.12/0.12・街区0.18/0.18。沈めないと駒が梢や屋根の上に立って見える。
...
- 【視覚】高い地形は、その奥の地面を隠す。俯角52°なので隠す奥行は 高さ×1/tan52°＝高さ×0.78（壁0.70なら0.55＝ヘックスの奥行 √3R の3割強）。地面に置く記号の見え方に効くので、盤の表示記号は [../gdd/uiux.md](../gdd/uiux.md) 側にこの制約を書いている。
