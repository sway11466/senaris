...
柵や道のように隣と繋がって見える地形は、隣り合う同スキンとの繋がり方でタイルの絵が変わる。terrain_skin.csv の `connect` 列に `line`（線＝柵）か `area`（面＝道）を書くと、盤（[hex_board_3d.gd](../../presentation/board/hex_board_3d.gd)）と [map_editor](../tech/tools.md) が向きの組み合わせ別のタイルを引く。繋がらない地形は `false`。
...
- 盤の縁での繋がり判定は、線と面で分ける（`connect` が `line` か `area` か）。どちらも狙いは同じで、地図の外側にも世界が続いている扱いにし、縁に輪郭付きの蓋ができるのを防ぐ。
  - 線（柵）: 繋がっている隣が1つだけのマスで、その反対側が盤外ならそちらへ腕を伸ばす（[TerrainSkin.extend_off_board](../../data/terrain/terrain_skin.gd)）。隣が2つ以上あるマスには効かせない＝外周に沿って走る柵が外向きの腕を櫛のように生やすため。
  - 面（道）: 盤の外を「縁のマスがそのまま続いている」として扱う。盤外にある隣の座標を盤の矩形へ丸め込み、その先のマスの地形を読む。帯が縁で切られず、そのまま盤の外へ抜ける。
