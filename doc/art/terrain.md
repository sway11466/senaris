...
- 反復対策＝向きのばらし: `terrain_skin.csv` の `orientable` で、同じ絵をヘックス座標から決定的に回す／反転する。どこまで許せるかは絵の描き方で決まる。
  - `full`＝60°回転＋左右反転。真上から見たテクスチャで、どの向きでも成立する絵（平地・森・荒地・茂み・山・台地）。
  - `flip`＝左右反転だけ。真上視点の地面に、立てた物（墓標）を描いた絵。回すと物が倒れる。影を描くと反転で光源が隣と食い違うので、この種の絵には影を描かない。
...
- 面方式は道専用ではない。接続タイルを引くのはスキンの `connect` 列だけを見る仕組みなので、地形タイプを問わず使える（菜園 `bush_garden1`＝茂みスキンを面方式で作る。畝の縞の元絵1枚から64枚）。区画の縁が街路や地面できっちり切れ、孤立した1マスは角の取れた塊になる。
- ただし面方式の地形は高さを持てない（§1）＝平らな地形に向く。厚みで見せたい地形は一面テクスチャ側（§2）を選ぶ。街区を面方式にすると縁は綺麗に切れるが建物の壁面が出せない、というのがこの分かれ目。
- 面方式はタイルを回さない（マスクだけが変わる）ので、畝のような向きのある柄は全マス同じ向きに揃う。ひとつながりの区画としては正しい見え方で、別々の区画に見せたいなら一面テクスチャ＋回転を選ぶ。
