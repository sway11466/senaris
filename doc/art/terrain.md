...
- ファイル名は `assets/terrain/{skin_id}_c{6桁}.png`。6桁は [Hex.DIRECTIONS](../../domain/hex/hex.gd) の順で、1＝その方向の隣も同じ名札。64通り。欠けている組み合わせは `{skin_id}.png` に落ちる（どちらの方式も直線 `_c001001` を複製して置く）。
- 「同じ隣」の判定は skin_id ではなく名札（terrain_skin.csv の `connect_as`・空なら skin_id）で行う。別スキンを同じ地形として繋ぎたいときに書く（川と橋に `water`）。名札は片方向＝隣から見た自分の数えられ方だけを決め、自分がどの絵を引くかには関わらない。
...
橋は「下に川、上に板」の2枚重ねで作る。板の絵は接続で選ばず、架かる軸で決める。

- ヘックスの軸は3本（縦＝`001001`／右上－左下＝`010010`／左上－右下＝`100100`）。橋はこの3本ぶんのスキンに分ける（`bridge` / `bridge_r` / `bridge_l`）。作者がマスごとに選ぶ＝向きは推測しない。地形タイプは1つのままなので、性能表も movement.csv も1行1列で済む。
- 橋スキンは `connect` を書かない（＝繋がらない）。自分の絵が接続で決まらないので、隣の川に向かって板が腕を伸ばすことがない。長い橋は同じ軸のスキンを並べるだけで1本に見える（板は辺の中点まで届いている）。
- 名札に `water` を書く（§3.1）。これで両隣の川が橋のマスへ帯を伸ばす。橋のマスの下に敷く川は `map_ground` が向き別に引く（§3.6）。
- 板の元絵は線方式（§3.2）で、地面を焼き込まず `-NoBase` かつ `-NoShadow` で書き出す。生成される64枚のうち使うのは上の3枚だけ。影を焼くと3方向で光の向きが食い違う。
- 高さは持たせない。高さはマス単位なので、橋のマスを持ち上げると中の水も一緒に上がり、川に段差とスカートが出る（帯状の地形に共通の制約 → §1）。板だけを浮かせるには、タイル面の高さとユニットが立つ高さを分ける改修が要る。
