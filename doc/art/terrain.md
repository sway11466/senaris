...
- 元絵の条件: 縦一直線・左右中央・上下端まで貫通・全長どこも同じ姿・落ち影なし。縦横比は問わない（スクリプトは元絵の寸法を読まず、渡した矩形だけを切る）。影は生成時に一定方向で付ける（元絵に焼き込むと、回した腕の影が別方向を向く）。平らな地形は影そのものを出さない（`-NoShadow`）。
- 切り抜きは明るさで行う（`-Threshold`＝これより明るい画素を地面として捨てる）。**地形の絵は背景より暗いこと**が条件。背景は好きな色でよい＝下地は `plain.png` が担当し、元絵の背景は全部捨てられる。
  - 柵: 芝地（#B4C6A0・70%）に濃い木材。既定の閾値66%。
  - 道: 純白（100%）に砂色（89%）。`-Threshold 96`。明るい地形は芝地の上に描くと分離できないので、背景を白にする。
- 柱を持たない線地形（道）は切り出し区間（`-ArmFrom`／`-ArmTo`）が任意。全長が一様なのでどこを切っても繋がる。区間の長さが 110.85px に伸縮されるので、その比率で盤上の帯の幅が決まる。
- 面で繋がる線地形（道）は、**縁取りの内側で切り出す**。縁取りを含めると、6方向の腕を重ねたとき腕どうしの境目に縁が残って板を並べたように見える。柵は板が別々なので含めてよい。
- SUBJECT の置き場は `assets/terrain-src/{skin_id}/{skin_id}_prompt.txt`（柵・道とも）。
