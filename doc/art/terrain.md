...
- 【視覚】地形は盤上で標高を持てる（見た目のみ・性能不変）。マスの高さは 行の基準＋列の基準＋スキンの `elevation` の和で、前2つはステージが持つ（→ [../gdd/terrain.md](../gdd/terrain.md) 盤の高さ）。スキン側の高さは [terrain_skin.csv](../../data/terrain/terrain_skin.csv) の `elevation` 列＝地形タイプではなくスキンごとに持ち、[hex_board_3d.gd](../../presentation/board/hex_board_3d.gd) がその値でタイルを持ち上げ、低い隣接辺／盤外辺に崖のスカートを下ろしてメサに見せる。値の単位は TILE=1.0（中心〜頂点）。ユニット・影・グリッド・拠点・オーバーレイ・クリック判定（ピッキング）も標高に追従する。
- 【視覚】高さが「地形の厚み」なら、駒がその上に立つか中に立つかは `terrain_skin.csv` の `sprite_sink`（立ち絵の沈み）が決める。よじ登る地形（城壁・瓦礫の山）は沈み0＝上面に立つ。木々の間の地面に居るべき地形（森・茂み）は高さと同じ値だけ沈める。沈めないと駒が梢の上に立って見える。
...
- 反復対策＝バリアント敷き分け: 同名連番 `{name}_2.png` `{name}_3.png` … を置くと、hex_board が存在する分を集め、ヘックス座標から決定的に敷き分ける（ちらつかない）。連番が無ければ従来どおり1枚。terrain.csv/JSON は変更不要のドロップイン。
- 反復対策＝向きのばらし: `terrain_skin.csv` の `orientable` で、同じ絵をヘックス座標から決定的に回す／反転する。どこまで許せるかは絵の描き方で決まる。
  ...
  - `flip`＝左右反転だけ。真上視点の地面に、立てた物（墓標）を描いた絵は、回すと物が倒れる。塊に向きがある絵（茂み）も、回すと1マスごとに向きが変わって「別々の藪の寄せ集め」に見えるので回さない＝向きを揃えるとひとつながりの地帯として読める。影を描くと反転で光源が隣と食い違うので、この種の絵には影を描かない。
  - `flip_xy`＝上下左右の反転だけ・回転なし。向きを持つ絵（街区・菜園）に使う。回すと絵の向きが隣の街路と食い違うが、反転は軸を変えないので繰り返しを散らす手として残る。向きを作者が選ばせたいなら、向き別のスキンを並べて塗り分ける（街区）。
  - `none`＝どちらもしない。向きが意味を持つ絵（道・柵・壁・拠点）。接続タイルは向き別に絵を持つので、そもそも回してはいけない。
  ...
  - `flip`／`none` は同じ絵がそのまま隣り合うので、反復対策を変種の枚数で持つ必要がある（墓標は5枚）。
...
- 街区は「1ヘクス＝1街区」の物として描く（`plateau_town_v` / `_r` / `_l`／地形タイプは台地）。八〜十軒を1つの塊に詰め、塊の外と路地を透過にして、地面は `map_ground`（道）に敷かせる。家が切れず、街路で区切られた街区が並ぶ形になる＝実際の街の成り立ちと同じ。1ヘクス1軒にしないのは、城壁に囲まれた街ぜんぶに対して家が大きすぎ、盤が狭く見えるため。
- 街区は回さず、向き別に3スキンを置いて作者が塗り分ける（`orientable=flip_xy`）。フラットトップ六角形の軸は 縦／右上から左下／左上から右下 の3本しかないので、縦の元絵1枚を `-Rotate 60` と `-Rotate 120` で回せば3枚とも作れる。自動回転にすると、どのヘックスも同じ配置の回し違いに見えるうえ、街路の通し方を作者が決められない。
