...
- 【視覚】地形は盤上で標高を持てる（見た目のみ・性能不変）。高さは [terrain_skin.csv](../../data/terrain/terrain_skin.csv) の `elevation` 列＝地形タイプではなくスキンごとに持ち、[hex_board_3d.gd](../../presentation/board/hex_board_3d.gd) がその値でタイルを持ち上げ、低い隣接辺／盤外辺に崖のスカートを下ろしてメサに見せる。値の単位は TILE=1.0（中心〜頂点）。ユニット・影・グリッド・拠点・オーバーレイ・クリック判定（ピッキング）も標高に追従する。
...
- 反復対策＝向きのばらし: `terrain_skin.csv` の `orientable` で、同じ絵をヘックス座標から決定的に回す／反転する。どこまで許せるかは絵の描き方で決まる。
  - `full`＝60°回転＋左右反転。真上から見たテクスチャで、どの向きでも成立する絵（平地・森・荒地・山・台地）。
...
- 明暗むらの基準は「平らに見せたい地形」のもので、森の1.8が実績のある上限（平地・荒地は0.5〜0.9、茂み1.6）。起伏を見せる地形では逆に大きい方が働く＝大局の明暗が斜面の凹凸として読める。山は4.00をそのまま採用した。同じ数字が地形によって欠点にも長所にもなるので、平らか起伏かを先に決めてから当てる。
...
- 生成物の色は加工しない。明度が狙いから外れたら、SUBJECT の基準色 HEX を上げ下げして出し直す。明度は指定に追随して動くので、実測との差ぶんを指定に足せば当たる（山は指定 HSL 62 → タイル 51.8 ＝ 輪郭と陰影で10ポイント落ちた）。彩度は指定を下げても動かない（16枚出して下限が破れなかった）ので、外れたら選び直す。小物の数・構図・光の向きも同じくプロンプトでしか動かない。
