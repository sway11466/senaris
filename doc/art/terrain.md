...
- 【視覚】高さが「地形の厚み」なら、駒がその上に立つか中に立つかは `terrain_skin.csv` の `sprite_sink`（立ち絵の沈み）が決める。よじ登る地形（城壁・瓦礫の山）は沈み0＝上面に立つ。木々や建物の間の地面に居るべき地形（森・茂み・街区）は高さと同じ値だけ沈める。沈めないと駒が梢や屋根の上に立って見える。
...
- 反復対策＝向きのばらし: `terrain_skin.csv` の `orientable` で、同じ絵をヘックス座標から決定的に回す／反転する。どこまで許せるかは絵の描き方で決まる。
  ...
  - 旧データの `true`/`false` は「回してよいか」しか言えず `flip` と区別できないため、変換で弾く。実行時に未知の値が来たら `none` に倒す（黙って回して絵が倒れるより害が小さい）。上下反転が要るようになったら `TerrainSkin.ORIENTS` と `TerrainTiles.orient()` に1つずつ足す。
...
- 建物も面で描く（街区 `plateau_town1`／地形タイプは台地）。1ヘクスに1軒だと、城壁に囲まれた街ぜんぶに対して家が大きすぎ、盤が狭く見える。屋根を6〜10個、向きをばらして詰め、隙間に路地と中庭を置いた一面テクスチャにすると、隣接ヘックスがひとつながりの街並みになる＝森とまったく同じ効き方。引いた盤ではヘックス1枚が39×27pxなので屋根1つは10px弱＝個々の家は読めず「密集した屋根の面」として読める。屋根の向きが元からばらけているので `orientable=full` が使え、変種の枚数を回転で節約できる（墓標のように立てた物ではないので回して倒れない）。単体で1ヘクスを占めてよいのは、実際に一区画を占める建物（拠点の詰所など）だけで、それらは密集した屋根の面の中でランドマークとして浮く。
- 地面ではなく地面の上に載る物（菜園の畑・木枠つきの区画）は、面で覆わず1つの物として描き、周囲を透過にして地面は `map_ground` に任せる（§3.6 と同じ組み方）。切り抜きは `gen_terrain_tile.ps1` のスイッチ2つで済む。
  ...
  - `-Fit`（＋ `-FitMargin`）＝物を中央に寄せ、回転しても六角形からはみ出さない大きさになるまで元絵に余白を足す。寸法は物の対角の半分で決める。回転が掃くのがその範囲だからで、中心〜頂点の128pxを基準にすると中心〜辺（110.85px）で切られる。副産物として大きさが正規化され、生成のたびに余白が違っても変種の縮尺が揃う。
