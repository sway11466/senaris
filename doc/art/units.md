...
- 顔は敵味方を問わず怖くしない。共通STYLEの「穏やかな丸い顔」を敵にも適用し、怪物であることは体（翼・鉤爪・牙・体つき）で伝える。顔まで怪物側に振ると、胴との接合が浮くか、ただ不気味なだけの絵になる（ハーピー・ローグヘッドで戻した）。
  - 顔で悪辣さを出したいときも、痩けた頬・落ち窪んだ目・鉤鼻・牙・への字口・目元の影は使わない。動かすのは眉と目の開き方だけにする。
  - 「片眉を上げる＋半目＋片方だけ上がる口角」の3つが揃うと、狙いに関係なく小憎らしい顔になる。悪役を狙うときはこの3点を意識して使い、可愛げを残したいときは3点とも禁止する（ピクシーで踏んだ）。
...
- グループ＝**style.md の単位**。中に「陣営内アンカー」を1体だけ置き、他はそのアンカーへ言葉で寄せる。アンカーが2つになる粒度（例：ゴブリンとオークを1つの style.md に入れる）は避ける。
- 分類＝**味方は兵種・敵は素性**。値は英字id で、表示名は `unit_group.<id>.name`（[../tech/i18n.md](../tech/i18n.md)）から引く。CSV の行順がそのままパレットとクロニクルの並びになるので、分類ごとに行を固めておく。
  - 軸が敵味方で違うのは、覚え方が違うから。味方は自分が編成する駒なので役割（歩兵・弓兵…）で見る。敵は相手の一味として出てくるので素性（ゴブリン・アンデッド…）で見る。情報パネルが味方にだけ兵種を添えるのも同じ理由（[../gdd/uiux.md](../gdd/uiux.md) 見出し）。
  - 味方行の分類は `unit_type.csv` の `category` と一致させる。情報パネルの見出しはこの列だけを読むので、ずれると嘘の兵種が出る。`convert.gd` が一致を検証し、外れれば JSON を書かない。
  - 敵の素性は性能から導けないので、行ごとに人が決める。人間の一味（悪党）も素性のひとつ＝種族に限らない。
- 分類を戦闘・移動の判定に使わない。読むのはマップエディタのパレット絞り込みと、画面の見出し（クロニクル・情報パネル）だけ。
- **兵種を画面に出すのは味方だけ**。敵に出すとリスキン元の性能が透ける（[../gdd/uiux.md](../gdd/uiux.md) 見出し）。敵の分類は素性なので、そのまま出しても兵種は漏れない。
...
盤は「絵の高さ」ではなく「PNGのキャンバス高さ」を一定のタイル数に対応させて描く（`hex_board_3d.gd` の `UNIT_CANVAS_TILES`）。クロニクルの格子も同じ理屈で、キャンバスの下端から決まった px を紙の高さに当てる（[../gdd/chronicle.md](../gdd/chronicle.md) ユニット 見せ方）。大小関係はキャンバスに焼き込んだ余白で表しており、たとえばハーフリング（倍率0.7）は 384 の枠の中で絵が 140px しかないから小さく見える。したがって次の3つが守られていないと大小関係が壊れる。
...
戦闘立ち絵（3.3）は別のキャンバス（704・BaseHeight 384＝高さは倍率 1.83 まで）。こちらは幅もキャンバスで頭打ちになる（内接で嵌める）ので、横長の絵は倍率より先に幅で止まる＝現時点で幅を最も使うのはワイアーム（高さ499px×比1.32＝659px）。
