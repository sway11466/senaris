...
- 発動者ごとに絵を分けるレシピ（`cutin_per_caster`・④トリックショット）は `{recipe_id}_{skin}` を起点にし、スキンごとに1式を置く。例：`formations-src/trick_shot/trick_shot_archer_01_raw.png`・`trick_shot_archer_prompt.txt`・`formations/trick_shot_archer.png`。スキルだけの名前（`trick_shot.png`）は置かない＝置いても出ない。
