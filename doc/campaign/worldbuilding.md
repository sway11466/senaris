# 世界の作り方

世界を組み立てるときの決めごと。設定の中身は [world.md](world.md)、冒険譚の書き方は [authoring.md](authoring.md) に置き、ここには「設定を増やすときに何を守るか」だけを置く。

---

## 役割で世界を見せる

固有名を持たせない。ユニット・登場人物・神のいずれも、役職や称号で呼ぶ（騎士・森の聖女・秩序の神）。

- 理由: ステージの会話は短く、名を覚えさせる尺がない。盤の駒が兵種で見える以上、物語も同じ粒度で見せるほうが一貫する。
- 英語では the ＋ 大文字の称号（the Order, the Forest, the Fickle One）で固有名詞として成立する。先例は『ゲーム・オブ・スローンズ』の七神（the Father, the Smith, the Stranger …）で、すべて役割名で通っている。
- 国も同じ。「冒険者の国」「聖職の国」のように、その国で厚い組織で呼ぶ。
- 陣営が違えば同じものの呼び名が変わる（秩序側の「邪神」＝混沌側の「対の神」）。呼び名の違いそのものが設定になる（[twingods.md](twingods.md) 陣営で言葉が変わる）。

## 陣営の色付け

- 舞台はファンタジー（剣と魔法）。
- 味方＝人・種族・職能／敵＝魔物・闇の種族。
- 人間の悪党も敵になる（冒険譚3のローグ一味）。善悪は種族で固定ではない。
- **盤の上の敵は迷いなく敵。** 撃つことにプレイヤーが躊躇する構図は作らない。一行が戦う理由は、国や教義の思惑の外に置く（双子の神の「一行が止める理由は神学の外にある」、古代遺跡の「一行が戦う理由は国の思惑の外にある」）。
- **深みは「なぜそうするか」に置き、1〜2行で見せる。** 説明しない。敵の頭目の台詞1本と、味方の誰かが漏らす一言で足りる。会話は短くスキップもできる（[authoring.md](authoring.md) 会話パート）ので、読まないと分からない形では出さない。

## 国は厚い組織と主神で描く

- 世界には複数の国があり、国ごとに厚い組織が違う。それがその国の色になる。
- 国ごとに主に祀る神も違い、厚い組織と主神は対応させる（教会＝秩序、魔法ギルド＝知識、商人＝商売と海、軍＝戦）。
- 主教を持たない国があってよい（冒険者の国）。国に依らず各地に信者を持つ神もいる（気まぐれの神・音楽と快楽の神）。
- 神々の間に上下や序列を置かない。双子（秩序・混沌）も同じで、片方が悪なのではなく、国がどちらを祀るかが違うだけ。

## 世界地図は描かない

- 国の位置関係は隣接の一覧（[world.md](world.md) 国の隣接関係）で持ち、国境をまたぐ冒険譚を書くときに略図を起こす。
- 方角や距離を台本に出さない。地図を縛ると後の冒険譚が置けなくなる。

## 設定は冒険譚から起こす

- 世界観の設定は、冒険譚で必要になったものを [world.md](world.md) に書く。先に世界を作り込まない。
- 構想中の冒険譚から来た設定は、その旨を添える（[roadmap.md](roadmap.md)）。

## 関連ドキュメント

- [world.md](world.md) — 設定の中身（国・神々・種族・脅威）
- [authoring.md](authoring.md) — 冒険譚の制作方針（登場人物・会話パート）
- [twingods.md](twingods.md) — 秩序と混沌の骨格
