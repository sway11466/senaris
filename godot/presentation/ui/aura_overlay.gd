...
## 陣営全体バフ（グレイス等）が効いている間、画面の外周から光を差し込ませる。
## 盤の情報層（地形・駒・範囲塗り）に一切触れないのが狙い＝中央は素通しで、縁だけ明るい。
## 画面全体に敷く。盤エリアだけに敷くと、盤の右端に光の帯が立って断ち切られて見えるため。
## 置き場所は ScreenLighting（画面の明暗の共通基盤・暗幕より前）＝暗転中も縁の光が生きる。
## 前面パネル層（InfoPanel など）は光より前に浮く＝光はパネルの後ろへ回り込む形になる。
