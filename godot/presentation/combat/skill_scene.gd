...
## detail は FormationResolver.resolve の cast（SkillCast）。
...
func play(detail: SkillCast) -> void:
	if detail == null:
		return
	var caster := detail.caster
	var victim := detail.target
...
func _skill_effect(detail: SkillCast, caster: UnitSnapshot) -> CombatEffect:
	var id := detail.combat_effect
	if id != "":
		return CombatEffectCatalog.by_id(id)
	return _effect_of(caster)

## 対象に何が起きたかの表示。損害数の代わりに、乗った補正（または落とした件数）を出す。
func _effect_text(detail: SkillCast) -> String:
	# 継続ダメージ（ポイズンスティング）は掛けた瞬間には減らない＝兵量バーが動かない。
	# その代わり、毎ターン何人減るかをここに出す。詳細 → doc/gdd/skills.md
	if detail.effect == "dot":
		var per_turn := detail.dot_troops
	...
	if rows.is_empty():
		return ""
	var v := detail.value
	var body := ("×%.2f" % v) if detail.op == "mul" else ("%+d" % int(round(v)))
