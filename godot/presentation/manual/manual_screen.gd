...
##
## 左が目次、右が本文。節が2つ以上ある章（いまは敵AIだけ）は目次が1段潜って節の一覧になり、
## 先頭の「← 章の一覧」で戻る。章の一覧へ戻す役をこの行が持つので、左下の戻ると Esc は
## どちらも常に画面を閉じる＝隅のボタンはどの画面でも「この画面を出る」の一つだけになる。
##
...
## 目次の行は、いちばん長い一覧（敵AIの節＝章へ戻る行＋節11）が縦に収まる寸法にしてある。
...
var _in_sections := false  ## 目次が節の一覧まで潜っているか
...
## 目次。章の一覧か、節が2つ以上ある章に潜ったときはその節の一覧。
func _build_toc() -> void:
	_clear(_toc_box)
	if not _in_sections:
		for i in ManualToc.CHAPTERS.size():
			var id := String(ManualToc.CHAPTERS[i]["id"])
	...
	_toc_box.add_child(_toc_nav(tr("ui.manual.chapters")))
	for i in chapter["sections"].size():
		var sid := String(chapter["sections"][i]["id"])
		var key := ManualToc.section_title_key(String(chapter["id"]), sid)
		_toc_box.add_child(_toc_row(tr(key), i == _section, _on_section.bind(i)))
...
## 章の一覧へ戻す行。同じ画面の中を繰るだけの道具なので、板ではなく無機質なグレーで出す
## （キャンペーンセレクトの繰り矢印と同じ側）。
func _toc_nav(text: String) -> Control:
	var b := TavernTheme.nav_button(text, TOC_FONT_SIZE)
	b.alignment = HORIZONTAL_ALIGNMENT_LEFT
	b.custom_minimum_size = Vector2(0, TOC_BUTTON_HEIGHT)
	b.pressed.connect(_on_chapters)
	return b
...
func _build_body() -> void:
	...
	var title := _label(tr(ManualToc.section_title_key(chapter_id, section_id)), SECTION_FONT_SIZE, ACCENT)
	_body_box.add_child(title)
...
## 章を選んだ。節が2つ以上ある章なら目次を節の一覧まで潜らせる。
func _on_chapter(index: int) -> void:
	SfxPlayer.play_event("menu_command")
	_chapter = index
	_section = 0
	_in_sections = ManualToc.CHAPTERS[index]["sections"].size() > 1
	_rebuild()
