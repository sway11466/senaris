...
##
## 形ごとに配置を1つ決めて描く。盤と同じフラットトップの axial（Hex.to_pixel）。斥候型・背後型は
...
func setup(shape: String, textures: Array) -> void:
	_shape = shape
	_textures = textures
	queue_redraw()
...
func _draw() -> void:
	var hexes: Array = LAYOUTS.get(_shape, [])
	var n := mini(hexes.size(), _textures.size())
	if n == 0:
		return
