...
## 見出しの文字列＝ユニット名（兵種）。兵種を持たない駒は名前だけ。
func _unit_title(skin: UnitSkin) -> String:
	var name_text := tr("unit.%s.name" % skin.skin_id)
	var category := UnitCatalog.display_category(skin.type_id)
	if category.is_empty():
		return name_text
	return tr("ui.chronicle.name_category") % [name_text, tr("category.%s.name" % category)]
