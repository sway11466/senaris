...
static func resolve(state: BattleState, option: Dictionary, target: Vector2i) -> Dictionary:
	...
	for pid in option["participants"]:
		...
		if p == null or p.team != state.current_team:
			return {}
		if not state.has_action_left(int(pid)):
			return {}
	...
	var caster_snap := {}
	var caster := state.unit_by_id(int(option.get("leader_id", -1)))
	if caster != null:
	...
	# 着弾・兵数変化は起きない。湧いた位置は cells で盤に返す＝光らせる。詳細 → doc/gdd/skills.md
		...
		var spawned := state.spawn_unit(option)
		if spawned != null:
	...
	for pid in option["participants"]:
		...
		state.set_done(int(pid))
		state.mark_engaged(int(pid))
	...
	if int(recipe_def.get("charge_turns", 0)) > 0:
		state.set_charge(int(option["leader_id"]), rid, 0)
...
static func _skill_snapshot(state: BattleState, option: Dictionary, target: Vector2i) -> Dictionary:
	var caster := state.unit_by_id(int(option.get("leader_id", -1)))
...
static func _dot_entry(state: BattleState, option: Dictionary, target: Vector2i) -> Dictionary:
...
## 詳細 → doc/gdd/formations.md, doc/gdd/skills.md
static func _buff_entry(state: BattleState, option: Dictionary, target: Vector2i) -> Dictionary:
