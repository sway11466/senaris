...
## 移動距離の表＝unit_id の駒の移動規則で、from を起点に流した道のり表。
...
static func terrain_cost_field(state: BattleState, unit_id: int, from: Vector2i) -> Dictionary:
	var u := state.unit_by_id(unit_id)
	if u == null:
		return {}
...
static func detour_cost_field(state: BattleState, unit_id: int, from: Vector2i,
...
static func move_distance(state: BattleState, unit_id: int, cells: Array) -> int:
	var u := state.unit_by_id(unit_id)
	if u == null:
		return BattleState.UNREACHABLE
	return min_cost_in(move_cost_field(state, unit_id, u.pos), cells)

## 地形距離＝移動距離から駒の除外を外し、地形だけで測った最小距離。
static func terrain_distance(state: BattleState, unit_id: int, cells: Array) -> int:
	var u := state.unit_by_id(unit_id)
	if u == null:
		return BattleState.UNREACHABLE
	return min_cost_in(terrain_cost_field(state, unit_id, u.pos), cells)

## 迂回距離＝移動距離に敵ZOCの除外を加えた最小距離。ignore_zoc_id は detour_cost_field と同じ。
static func detour_distance(state: BattleState, unit_id: int, cells: Array,
		ignore_zoc_id: int = -1) -> int:
	var u := state.unit_by_id(unit_id)
	if u == null:
		return BattleState.UNREACHABLE
	return min_cost_in(detour_cost_field(state, unit_id, u.pos, ignore_zoc_id), cells)
...
static func detour_cost_field_to(state: BattleState, unit_id: int, from: Vector2i,
		target_id: int) -> Dictionary:
	return detour_cost_field(state, unit_id, from, target_id)

## 標的がユニットのときの迂回距離＝その標的に攻撃可能なマスまで、標的自身のZOCだけ外して測る。
static func detour_distance_to(state: BattleState, unit_id: int, target_id: int) -> int:
	return detour_distance(state, unit_id, state.attack_cells(unit_id, target_id), target_id)
