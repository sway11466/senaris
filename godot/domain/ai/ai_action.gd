...
var kind: Kind
var handle: int         ## 行動する駒。UNLOAD では降ろす側＝輸送の id
...
static func move_to(handle: int, to: Vector2i) -> AiAction:
	var a := AiAction.new()
	a.kind = Kind.MOVE
	a.handle = handle
	a.to = to
...
static func attack(handle: int, target_id: int) -> AiAction:
	var a := AiAction.new()
	a.kind = Kind.ATTACK
	a.handle = handle
...
## handle が option のユニットスキルを target へ放つ1手（詳細 → doc/gdd/ai.md 特性詳細のスキルの行）。
## option は Formation.available_for が返すものそのまま＝application が FormationCommand へ翻訳する。
static func skill(handle: int, option: FormationOption, target: Vector2i) -> AiAction:
	var a := AiAction.new()
	a.kind = Kind.SKILL
	a.handle = handle
	a.option = option
...
static func enter_base(handle: int) -> AiAction:
	var a := AiAction.new()
