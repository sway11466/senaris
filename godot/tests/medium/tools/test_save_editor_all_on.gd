...
## 実データ（冒険譚1のマニフェスト・スキン表・レシピ）を、テスト用の置き場に置いたストアへ書く。
...
func test_all_on_records_every_skin_and_recipe() -> void:
	var n := _run()
	...
	var recipes := chronicle.recipes()
	assert_eq(n["recipes"], SaveEditorModel.formation_recipe_ids().size(), "全陣形スキルを書いた")
	for recipe_id in SaveEditorModel.formation_recipe_ids():
		assert_true(recipes.has(recipe_id), "レシピ: %s" % recipe_id)
		assert_eq(recipes[recipe_id]["first"], "", "初出の冒険譚は空")
...
func test_all_on_is_idempotent() -> void:
	...
	assert_eq(second["recipes"], 0, "2回目は新規に書くレシピが無い")
