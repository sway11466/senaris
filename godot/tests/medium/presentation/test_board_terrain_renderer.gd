...
func _setup_height(overrides: Dictionary, skins: Dictionary = {}) -> void:
	renderer.setup(BattleState.new(4, 4), skins, {}, overrides)

func test_elev_is_skin_elevation() -> void:
	var hex := Hex.offset_to_axial(1, 1)
	_setup_height({}, { hex: "plateau" })
	assert_almost_eq(renderer.elev(hex), TerrainSkinCatalog.resolve("plateau", "plateau").elevation, 0.001,
		"上書きが無ければスキンの elevation")

func test_height_override_replaces_skin_height() -> void:
	var hex := Hex.offset_to_axial(1, 1)
	_setup_height({ hex: { "elevation": 0.5, "floor": 0.25 } }, { hex: "plateau" })
	assert_almost_eq(renderer.elev(hex), 0.5, 0.001, "上書きした値がそのまま最終の高さ＝スキンの elevation は見ない")
	assert_almost_eq(renderer.unit_floor(hex), 0.25, 0.001, "floor も上書きした値がそのまま")

func test_height_override_is_per_cell() -> void:
	var hex := Hex.offset_to_axial(1, 1)
	_setup_height({ hex: { "elevation": 0.5, "floor": 0.5 } })
	assert_almost_eq(renderer.elev(Hex.offset_to_axial(1, 2)), 0.0, 0.001, "隣のマスはスキンのまま")
