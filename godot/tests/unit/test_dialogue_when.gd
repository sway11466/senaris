...
func _stage() -> Dictionary:
	return { "dialogue": { "intro": [
		{ "speaker": "char.van.name", "skin": "knight", "text": "dragon-hunt.st5.van.01" },
		{ "speaker": "char.elf.name", "skin": "elf", "text": "dragon-hunt.st5.elf.01", "when": "joined:t3.elf" },
		{ "speaker": "char.van.name", "skin": "knight", "text": "dragon-hunt.st5.van.02", "when": "!joined:t3.elf" },
	], "outro": [] } }
...
func test_no_roster_keeps_only_unconditional_and_negated_lines() -> void:
	...
	assert_eq(_texts(d["intro"]), ["dragon-hunt.st5.van.01", "dragon-hunt.st5.van.02"], "名簿なし＝未加入として扱う")
...
func test_joined_actor_shows_their_line() -> void:
	...
	assert_eq(_texts(d["intro"]), ["dragon-hunt.st5.van.01", "dragon-hunt.st5.elf.01"], "在籍していれば喋り、否定の行は落ちる")

func test_zero_troops_member_still_speaks() -> void:
	# 兵力ゼロは戦線離脱であって死亡ではない＝会話には出る。
	var roster: Array = [{ "type": "elf", "troops": 0, "actor": "t3.elf" }]
	var d := StageLoader.parse_dialogue(_stage(), roster)
	assert_true("dragon-hunt.st5.elf.01" in _texts(d["intro"]), "離脱者も会話に出る")

func test_other_actor_does_not_satisfy_condition() -> void:
	var roster: Array = [{ "type": "knight", "troops": 8, "actor": "t3.van" }]
	var d := StageLoader.parse_dialogue(_stage(), roster)
	assert_eq(_texts(d["intro"]), ["dragon-hunt.st5.van.01", "dragon-hunt.st5.van.02"], "別人の在籍では条件を満たさない")
