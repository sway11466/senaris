...
func test_save_persists_across_instances() -> void:
	...
	assert_false(loaded.is_empty(), "tutorial3 にスナップショットがある")
	...
	assert_eq(reloaded.load_roster("other"), [], "冒険譚IDで区別する")
...
func test_clear_removes_snapshot() -> void:
	...
	assert_eq(store.load_roster("tutorial3"), [], "破棄で消える")
	assert_eq(RosterStore.new(PATH).load_roster("tutorial3"), [], "破棄が保存されている")
...
func test_non_array_entry_skipped_others_survive() -> void:
	...
	var store := RosterStore.new(PATH)
	assert_eq(store.load_roster("broken"), [], "配列でないエントリはスキップ")
	assert_false(store.load_roster("tutorial3").is_empty(), "正常なエントリは残る")
