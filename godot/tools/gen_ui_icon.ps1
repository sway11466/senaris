...
.DESCRIPTION
  For one or more icon ids (or 'all'), reads assets/ui-src/<group>/<id>/<id>_01_raw.(png|jpg)
  and writes assets/ui/<group>/<id>.png: turn the flat single-color background into alpha,
  ...
  Recipe of record: doc/art/ui.md. Requires ImageMagick (magick).
  NOTE: keep this file ASCII-only (PowerShell 5.1).

.EXAMPLE
  powershell -File tools\gen_ui_icon.ps1 charge
  powershell -File tools\gen_ui_icon.ps1 charge guard raid
  powershell -File tools\gen_ui_icon.ps1 all
#>
param(
...
$IconIds = @($IconIds)
if ($IconIds.Count -eq 0) { throw "usage: gen_ui_icon.ps1 <icon_id> [<icon_id> ...] | all" }

$here = if ($PSScriptRoot) { $PSScriptRoot } else { Split-Path -Parent $MyInvocation.MyCommand.Definition }
$repo = Split-Path -Parent $here
$srcRoot = Join-Path $repo 'assets\ui-src'
$outRoot = Join-Path $repo 'assets\ui'

if (-not (Get-Command magick -ErrorAction SilentlyContinue)) {
  throw "ImageMagick (magick) not found. Install: winget install ImageMagick.ImageMagick"
}

# Icon folders are assets/ui-src/<group>/<id>/ ; the group becomes assets/ui/<group>/.
...
foreach ($id in $IconIds) {
  ...
  if (-not $dir) {
    Write-Warning "${id}: no folder under assets/ui-src -> skipped"
    continue
  }
  ...
  Write-Host ("{0,-10} {1,-7} -> assets/ui/{2}/{3}.png  {4}" -f $id, $from, $group, $id, $size)
}
