...
| `grace` | グレイスの発動 | 自作 | MuseScore（MS Basic 音源） | 自作物＝商用可・改変可・表記不要 | 全部自作 |
...
| `shield_wall` | シールドウォールの発動（弾き返しと同一素材） | 同上 | 同上 | 同上 | 同上、-8.7 dB。shield_wall_recipe.txt |
| `backstab_hit` | バックスタブの着弾（斬撃・小と同一素材） | Sonniss GDC 2026 / 344 Audio - Historical Weapons Vol. 2 `WEAPSwrd_Sword Slide Cuts, Metallic, Impact CM4 2` | wav 192kHz/24bit | 商用可・改変可・表記不要／再配布不可・AI学習不可 | `slash_s` と同一の切り出し（8.46〜9.06秒）、-1.3 dB。書き出しで頭に0.3秒の無音（`gen_sfx.ps1 -LeadSec 0.3`）。backstab_hit_recipe.txt |
...
| `trinity_nova` | トリニティノヴァの発射 | Sonniss GDC 2018 / Gamemaster Audio - Magic and Spell Sounds `water_blast_projectile_spell_03` | wav 96kHz/24bit | 同上 | 切り出しなし、-9.0 dB。trinity_nova_recipe.txt |
| `trinity_nova_hit` | トリニティノヴァの着弾 | Sonniss GDC 2020 / David Dumais Audio - Spells Magic 1 `Magic_Spells_Impact_Creation20` | wav 96kHz/24bit | 同上 | 切り出しなし、-9.0 dB。trinity_nova_hit_recipe.txt |
...
| `move_drift` | 漂う。ゴースト・レイスの移動（続く型＝動いている間ループ） | Sonniss GDC 2018 / Soundrangers - Wind `wind_harsh_mountain_02_30_loop` | wav 96kHz/24bit | 同上 | ループの一定な区間（30.70〜32.20秒・両端 0.03 秒フェード）、-1.5 dB。move_drift_recipe.txt |
| `move_wheel` | 車輪。馬車の移動（続く型＝動いている間ループ） | Sonniss GDC 2023 / Dramatic Cat - Horse Carriage - Draft Horse `VEHWagn_Wood Cart Roll On Stone Pavement In Courtyard 03_DRCA_HOCA_Kmr81i` | wav 96kHz/24bit | 同上 | 転がりの一定な区間（1.25〜2.75秒・両端 0.03 秒フェード）、-4.3 dB。move_wheel_recipe.txt |
| `move_wagon` | 馬車。車輪に蹄を重ねた音（続く型＝動いている間ループ） | Sonniss GDC 2023 / Dramatic Cat - Horse Carriage - Draft Horse `VEHWagn_Wood Cart Roll On Stone Pavement In Courtyard 03_DRCA_HOCA_Kmr81i` ＋ TheWorkRoom Audio Post - Animal Footsteps Foley - Hooves `ANF082` | wav 96kHz/24bit | 同上 | 車輪 1.25〜2.69秒（+1.7 dB）に蹄2打（`move_hoof` と同じ切り出し、-13.0 dB）を 0.48 秒ごとに3回重ね、全体 -2.9 dB。書き出しは `-SilenceDb -120dB`（ループ長 1.44 秒を保つ）。move_wagon_recipe.txt |
| `move_hoof` | 蹄。馬の移動（周期の型＝4マスごと） | Sonniss GDC 2023 / TheWorkRoom Audio Post - Animal Footsteps Foley - Hooves `ANF082` | wav 96kHz/24bit | 同上 | 2テイク（12.038〜12.228秒・7.776〜7.994秒）を 0.16 秒ずらして重ね、-7.0 dB。move_hoof_recipe.txt |
