...
const COLOR_ENEMY_REACH := Color(0.95, 0.35, 0.30, 0.22)  # 敵の移動（脅威）範囲。自軍の緑と区別
...
## 敵など操作できないユニットを閲覧（選択状態にはしない＝コマンドは出さない）。
## 参考として移動範囲＝脅威範囲だけは別色でハイライトする。
...
func _draw_tile(hex: Vector2i) -> void:
	...
	if _inspect_reach.has(hex):
		draw_colored_polygon(pts, COLOR_ENEMY_REACH)
