extends SceneTree
## 使い捨て：略奪品の山 (plain_cave1_prop_loot1) の戦闘中の見え方を撮る。
## 手前の帯（_combat_front）は略奪品に立つ側の半面だけに出る。右の駒が立つ／左の駒が立つ の2通り。
## 実行: godot --path . -s res://tests/manual/shot_loot_combat.gd -- <出力ディレクトリの絶対パス>
## （--headless は付けない＝描画されないと get_texture() が撮れない）

const LOOT := Vector2i(6, 5)   # 略奪品のマス
const FIELD := Vector2i(5, 5)  # 反対側のマス（洞窟の床）

const LOOT_R := { "team": 1, "type_id": "cleric", "skin_id": "goblin", "terrain": "prop", "pos": LOOT, "troops_before": 8 }
const LOOT_L := { "team": 0, "type_id": "fighter", "skin_id": "fighter", "terrain": "prop", "pos": LOOT, "troops_before": 8 }
...
func _initialize() -> void:
	...
	_stage.bind_terrain_skins({ LOOT: "plain_cave1_prop_loot1", FIELD: "plain_cave1" })
...
func _save(what: String) -> void:
	...
	var path := "%s/loot_combat_%s.png" % [_dir, what]
	img.save_png(path)
	print("saved: ", path)
